...
//		判断是否经过搜索的列表
		if(!url){
			var TableUrl = window.deviceURL+"hardware/device/findAll?deviceType="+WIRE.deviceType+"&pageSize=10";
		}else{
			var TableUrl = url;
		}
        ...
        <div class="row">
		    <div class="CameraButtons" style="margin-left:15px; margin-bottom:10px;">
			    <a class="btn btn-social btn-success" onclick="IMU.createIMUModale(WIRE)" data-toggle="modal" data-target="#modal-default">
	                <i class="fa fa-plus"></i> 新增天线
	            </a>
		    </div>
            ...
            <div class="col-xs-12">
                <div class="box box-primary box-solid">
			        ...
			        <div class="searchType" style="position: absolute;z-index: 1000;width:500px;padding: 10px;">
			            ...
			            <input type="text" class="form-control searchTypeValue" placeholder="回车搜索" onfocus="this.placeholder=''" onblur="this.placeholder='回车搜索'" onkeydown="IMU.Search(WIRE,event)" style="float:left;width:130px;margin:0px 5px;" />
			        </div>
				    <div class="box-body">
...
//	              		判断设备是否已经被绑定----1为绑定 - 0为未被绑定  -----绑定之后的硬件可以修改，但不可以删除
		    			if(row.bindFlag == "1"){
					    	return	'<a class="btn btn-primary" title="修改" href="#" onclick="IMU.Change(WIRE,'+data+')" data-toggle="modal" data-target="#modal-default" style="margin-right:3px;"><i class="glyphicon glyphicon-edit"></i></a>'
					    	+'<a class="btn btn-danger" title="硬件已被绑定" href="#" disabled="disabled"><i class="glyphicon glyphicon-trash"></i></a>'
		    			}else{
						    return	'<a class="btn btn-primary" title="修改" href="#" onclick="IMU.Change(WIRE,'+data+')" data-toggle="modal" data-target="#modal-default" style="margin-right:3px;"><i class="glyphicon glyphicon-edit"></i></a>'
						    +'<a class="btn btn-danger" title="删除" href="#" onclick="IMU.Delete(WIRE,'+data+')" data-toggle="modal" data-target="#modal-default"><i class="glyphicon glyphicon-trash"></i></a>'
		    			}
