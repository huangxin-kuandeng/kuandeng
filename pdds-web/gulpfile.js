...
var sourceJs = ['./Apps/Dist/Js/disease_centre.js',
                           ...
                           './Apps/Dist/Js/projection/get_show_polygon.js',
                           './Apps/Dist/Js/web_init.js'];

gulp.task('min_css', function () {
    // 找到 Apps/Dist/css 下的所有 css 文件
    return gulp.src('./Apps/Dist/css/*.css')
	    .pipe(concat('main.css')) 					//合并css
	    .pipe(cleanCSS()) 							//压缩css
	    .pipe(gulpRename({ suffix: '.min' })) 		//设置压缩文件名
	    .pipe(gulp.dest('./Apps/Dist/css/'))
});

gulp.task('min_login_css', function () {
    // 找到 Apps/Dist/login 下的登录界面所需要的 css 文件
    return gulp.src('./Apps/Dist/login/login.css')
	    .pipe(cleanCSS()) 							//压缩css
	    .pipe(gulpRename({ suffix: '.min' })) 		//设置压缩文件名
	    .pipe(gulp.dest('./Apps/Dist/login/'))
});
