...
/*
 * @Author: tao.w
 * @Date: 2019-08-22 19:07:21
 * @LastEditors: tao.w
 * @LastEditTime: 2019-10-23 20:13:42
 * @Description: 
 */
...
Potree.loadPointCloud = function (path, name, callback,cloudUrl,isSqlite = false) {
...
/*
 * @Author: tao.w
 * @Date: 2019-08-09 16:25:11
 * @LastEditors: tao.w
 * @LastEditTime: 2019-08-15 15:15:28
 * @Description: 
 */
...
Potree.Shader = class Shader {
	...
	linkProgram() {
		...
		if (cached) {
            ...
            //TODO 不应该出现program 为空问题， 现在没查到原因
            // if(!this.program){
            //     return
            // }
...
/*
 * @Author: tao.w
 * @Date: 2019-06-13 17:17:03
 * @LastEditors: tao.w
 * @LastEditTime: 2019-12-19 16:41:15
 * @Description: 
 */
 ...
 */
Potree.POCLoader.load = function load(url, callback, dataUrl,isSqlite = false) {
    try {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
                ...
                if (dataUrl) {
                    pco.octreeDir = dataUrl + fMno.octreeDir; 
                } else if (fMno.octreeDir.indexOf('http') === 0) {
                ...
                if (fMno.pointAttributes === 'LAS') {
                    pco.loader = new Potree.LasLazLoader(fMno.version,_isSqlite);
                ...
                { // load root
                    ...
                    let root = new Potree.PointCloudOctreeGeometryNode(name, pco, boundingBox,_isSqlite);
                    root.level = 0;
...
/*
 * @Author: tao.w
 * @Date: 2018-03-12 03:22:02
 * @LastEditors: tao.w
 * @LastEditTime: 2019-08-23 17:53:24
 * @Description: 
 */
...
Potree.BinaryLoader = class BinaryLoader{
	...
	parse(node, _buffer){
		...
		let numPoints = _buffer.byteLength / node.pcoGeometry.pointAttributes.byteSize;
 ...
 * @Date: 2018-03-12 03:22:02
 * @LastEditors: tao.w
 * @LastEditTime: 2019-08-23 17:59:31
 * @Description: 
 */
...
Potree.GreyhoundBinaryLoader = class{
	...
	parse(node, _buffer){
		...
		let view = new DataView(
			_buffer, _buffer.byteLength - NUM_POINTS_BYTES, NUM_POINTS_BYTES);
 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2019-10-23 20:56:02
 * @Description: 
 */
...
Potree.LasLazLoader = class LasLazLoader {
	constructor (version,isSqlite = false) {
		if (typeof (version) === 'string') {
			this.version = new Potree.Version(version);
		} else {
			this.version = version;
	...
	load (node) {
		...
		if (this.version.equalOrHigher('1.4') && !this.isSqlite) {
...
/*
 * @Author: tao.w
 * @Date: 2019-09-04 16:28:04
 * @LastEditors  : tao.w
 * @LastEditTime : 2020-01-17 15:06:09
 * @Description: 
 */
/**
...
Potree.InputHandler = class InputHandler extends THREE.EventDispatcher {
    constructor(viewer) {
        ...
        this.mouseDoewnMouse = this.mouse.clone();
    ...
    onMouseClick(e) {
        ...
        if (this.mouseDoewnMouse && this.mouse
            && this.mouse.equals(this.mouseDoewnMouse)) {
            for (let inputListener of this.getSortedListeners()) {
                inputListener.dispatchEvent({
                    button: e.button,
                    event: e,
    ...
    onMouseDown(e) {
        ...
        if (e.button === THREE.MOUSE.RIGHT) {
            for (let inputListener of this.getSortedListeners()) {
                if (!(inputListener instanceof Potree.MeasuringTool))
                    inputListener.dispatchEvent({
                        button: e.button,
                        event: e,
                        type: 'mousedown',
        ...
        } else if (this.hoveredElements.length === 0 || (
            this.drag && this.drag.object && this.hoveredElements.filter((d) => {
                if (!d.object) return false;
                return d.object.parent === this.drag.object.parent;
            }).length > 0
        )) {
            for (let inputListener of this.getSortedListeners()) {
                inputListener.dispatchEvent({
        ...
        if (!this.drag) {
            ...
            if (target) {
                ...
                this.startDragging(this.hoveredElements[0] || null);
            }
        }
    ...
    onMouseUp(e) {
        ...
        let consumed = false;
        let consume = () => { return consumed = true; };
        if (this.hoveredElements.length === 0 || e.button === THREE.MOUSE.RIGHT || (
            this.drag && this.drag.object && this.hoveredElements.filter((d) => {
                if (!d.object) return false;
                return d.object.parent === this.drag.object.parent;
            }).length > 0
        ...
        if (this.drag) {
            let isDrawing = true;
            let _draging = true;
            let x1 = Math.abs(this.mouse.x - this.mouseDoewnMouse.x);
            let y1 = Math.abs(this.mouse.y - this.mouseDoewnMouse.y);
            if (x1 == y1 && x1 ==0) {
                _draging = false;
            }
            // if(this.viewer.measuringTool.measureName != '' && this.viewer.measuringTool.measureName != iD.data.DataType.LAMPPOST){
            if (this.viewer.measuringTool.measureName != '' && this.viewer.measuringTool._maxMarkers != 1) {
                let xOffset = Math.abs(this.mouse.x - this.mouseDoewnMouse.x);
                let yOffset = Math.abs(this.mouse.y - this.mouseDoewnMouse.y);
                if (Math.max(xOffset, yOffset) > this.offset) {
                    isDrawing = false;
                }
            }
            if (this.drag.object && isDrawing) {
                if (this.logMessages) console.log(`${this.constructor.name}: drop ${this.drag.object.name}`);
                this.drag.object.dispatchEvent && this.drag.object.dispatchEvent({
                    button: e.button,
                    type: 'drop',
                    drag: this.drag,
                    viewer: this.viewer
                ...
                for (let inputListener of this.getSortedListeners()) {
                    ...
                    if (this.hoveredElements[0] ) {
                        let obj = this.hoveredElements[0].object
                        let measure = obj.parent;
                        if(obj.type && obj.type == 'point'){
                            measure = obj;
                        }
                        inputListener.dispatchEvent({
                            type: 'edge_click',
    ...
    onMouseMove(e) {
        ...
        for (let inputListener of this.getSortedListeners()) {
            inputListener.dispatchEvent({
                ...
                viewer: this.viewer,
                mouse: this.mouse,
                object: this.drag && this.drag.object,
                hovered: this.getHoveredElement()
            });
        }

        if (this.scene.isPlanarView) {
            this.scene.currentCamera = Potree.utils.getCurrentCamera(this.mouse);
            if (_.include(['topCamera', 'sideCamera', 'frontCamera'], this.scene.currentCamera.name)) {
                // if (this.scene.currentCamera.name == 'topCamera' || this.scene.currentCamera.name == 'sideCamera' || this.scene.currentCamera.name == 'frontCamera') {
                this.viewer.setNavigationMode(Potree.PlanarControls);
                this.mouse.x = this.mouse.x - this.domElement.clientWidth / 2;
                x = x - this.domElement.clientWidth / 2;
                if (this.scene.currentCamera.name == 'topCamera') {
                    this.mouse.y += 15;
                    y += 15;
                } else if (this.scene.currentCamera.name == 'sideCamera') {
                    // this.mouse.y = (this.domElement.clientHeight - this.mouse.y) - (this.domElement.clientHeight / 3 + 60);
                    // y =(this.domElement.clientHeight - y) - (this.domElement.clientHeight / 3);
                    this.mouse.y = y - this.domElement.clientHeight / 3 + 18;
                    y = y - this.domElement.clientHeight / 3 + 18;
                } else if (this.scene.currentCamera.name == 'frontCamera') {
                    this.mouse.y = y - (this.domElement.clientHeight / 3 + this.domElement.clientHeight / 3 + 26) + 10;
                    y = y - (this.domElement.clientHeight / 3 + this.domElement.clientHeight / 3 + 26) + 10;
                }
            } else {
        ...
        if (this.drag) {
            ...
            this.drag.end.set(x, y);
            // let isDrawing = false;
            // if(this.viewer.measuringTool.measureName != ''){
            //     // let xOffset = Math.abs(this.mouse.x - this.mouseDoewnMouse.x);
            //     // let yOffset = Math.abs(this.mouse.y - this.mouseDoewnMouse.y);
            //     // if(Math.max(xOffset, yOffset) > this.offset){
            //         isDrawing =  true;
            ...
            // }
            if (this.drag.object) {
                let hasDraggable = this.drag.object.hasOwnProperty('draggable');
                if (!hasDraggable || (hasDraggable && this.drag.object.draggable)) {
    ...
    getHoveredElements() {
        ...
        for (let scene of scenes) {
            scene.traverseVisible(node => {
                if (node._listeners && node.visible && node.editable && !this.blacklist.has(node)) {
        ...
        // if (this.scene.isPlanarView) {
        // 	var camera = Potree.utils.getCurrentCamera(this.mouse),
        // 	width = camera.width,
        // 	height = camera.height;
        // } else {
        var camera = this.scene.getActiveCamera(),
            width = camera.width || this.domElement.clientWidth,
            height = camera.height || this.domElement.clientHeight;
        // }
        let ray = Potree.utils.mouseToRay(this.mouse, camera, width, height);

        let raycaster = new THREE.Raycaster();
        raycaster.ray.set(ray.origin, ray.direction);
        raycaster.linePrecision = 0.1;

        let intersections = raycaster.intersectObjects(interactables.filter(o => {
            return o.visible;
        }), false);
...
Potree.EarthControls = class EarthControls extends THREE.EventDispatcher {
	constructor (viewer) {
		...
		let drag = (e) => {
			...
			// let camera = Potree.utils.getCurrentCamera(mouse);
			let camera = this.scene.getActiveCamera();
			let domElement = this.viewer.renderer.domElement;

			if (e.drag.mouse === Potree.MOUSE.LEFT) {

				let ray = Potree.utils.mouseToRay(mouse, camStart, camera.width || domElement.clientWidth, camera.height || domElement.clientHeight);
				...
				this.scene.sideCamera.position.x = camera.position.x;
				this.scene.sideCamera.position.z = camera.position.z - 2;
				this.scene.sideCamera.rotation.z = -this.scene.cameraP.rotation.x * 2;
				this.scene.frontCamera.position.x = camera.position.x;

				this.scene.topCamera.position.x = camera.position.x;
				this.scene.topCamera.position.y = camera.position.y;
				// this.scene.frontCamera.position.y = camera.position.y;
			} else if (e.drag.mouse === Potree.MOUSE.RIGHT) {
				if (this.scene.isPlanarView) {
					return;
				}
				...
				let pivotToCam = new THREE.Vector3().subVectors(camera.position, this.pivot);
		...
		let onMouseDown = e => {
			...
			if (I) {
				...
				// var camera = Potree.utils.getCurrentCamera(e.mouse);
	...
	update (delta) {
		...
		// if (viewer.scene.isPlanarView) {
		// 	camera = Potree.utils.getCurrentCamera(this.mouse);
		// }
		...
		if (this.wheelDelta !== 0) {
			...
			if (I) {
				...
				let resolvedPos = new THREE.Vector3().addVectors(camera.position, this.zoomDelta);
				...
				let targetDir = new THREE.Vector3().subVectors(I.location, camera.position);
				targetDir.normalize();

				resolvedPos.add(targetDir.multiplyScalar(jumpDistance));
				// this.zoomDelta.subVectors(resolvedPos, view.position);
				this.zoomDelta.subVectors(resolvedPos, camera.position);
		...
		if (this.zoomDelta.length() !== 0) {
			...
			let newPos = new THREE.Vector3().addVectors(camera.position, p);
			if (this.viewer.isTopView && viewer.scene.view.pitch == -Math.PI / 2 && view.position.z == 49) {
				newPos.z = 49;
		...
		// viewer.pRenderer && viewer.pRenderer.render(viewer.scene.scenePointCloud, camera);
		// viewer.renderer && viewer.renderer.render(viewer.scene.scene, camera);

		if (this.pivotIndicator.visible) {
			let distance = this.pivotIndicator.position.distanceTo(camera.position);
			// let distance = this.pivotIndicator.position.distanceTo(camera.position);
		...
		/*{
			...
			this.scene.topCamera.position.x = camera.position.x;
			this.scene.topCamera.position.y = camera.position.y;
			if (this.scene.topCamera.newZoom != 0) {
				this.scene.topCamera.position.z = this.scene.topCamera.newZoom;
			}

			//侧视
			this.scene.sideCamera.position.x = camera.position.x;
			if (this.scene.sideCamera.newZoom != 0) {
				this.scene.sideCamera.position.z = this.scene.sideCamera.newZoom;
			}
			// this.scene.sideCamera.position.z = camera.position.z - 2;

			//正视
			this.scene.frontCamera.position.x = camera.position.x;
			if (this.scene.frontCamera.newZoom != 0) {
				this.scene.frontCamera.position.z = this.scene.frontCamera.newZoom;
			}
			...
			// if(this.mouse.x == -1 && this.mouse.y == -1) {
				...
				this.scene.topCamera.rotation.order = "ZXY";
				if (this.scene.topCamera.newZoom == 0) {
					this.scene.topCamera.position.z = 80.55421064423958;//高度
				} else {
					this.scene.topCamera.position.z = this.scene.topCamera.newZoom;
				}
				// this.scene.topCamera.up = new THREE.Vector3(0,0,-1);
				// this.scene.topCamera.lookAt( new THREE.Vector3(0,-1,0) );
				// this.scene.topCamera.rotation.x = -0.029203673205103725;//旋转弧
				// // this.scene.topCamera.rotation.y = 0.2;
				// this.scene.topCamera.rotation.z = -0.5;//pitch
				this.scene.topCamera.rotation.z = this.scene.cameraP.rotation.z;
				
				//this.scene.view.yaw = 0;
				//this.scene.view.pitch
				// this.scene.topCamera.zoom = 25;

				// this.scene.sideCamera.position.copy(camera.position);
				
				this.scene.sideCamera.rotation.order = "ZXY";
				this.scene.sideCamera.position.copy(camera.position);
				this.scene.sideCamera.rotation.copy(camera.rotation);

				this.scene.sideCamera.rotation.y = -90 * Math.PI / 180;
				// this.scene.sideCamera.rotation.x = Math.PI / 2 + this.scene.view.pitch;
				// this.scene.sideCamera.position.x = camera.position.x;
				// this.scene.sideCamera.position.y = camera.position.y + 13;
				// if (this.scene.sideCamera.newZoom == 0) {
				// 	this.scene.sideCamera.position.z = camera.position.z - 2;
				// }
				// this.scene.sideCamera.rotation.x = camera.rotation.x;
				// // this.scene.sideCamera.rotation.y = camera.rotation.y - 1.4;
				// this.scene.sideCamera.rotation.z = -viewer.scene.cameraP.rotation.x * 2;
				// this.scene.sideCamera.rotation.x = 1.7000000000000004;
				
				// this.scene.sideCamera.position.z = 80.55421064423958;//高度
				// this.scene.sideCamera.zoom = 70;

				
				this.scene.frontCamera.rotation.order = "ZXY";
				this.scene.frontCamera.position.copy(camera.position);
				this.scene.frontCamera.rotation.x = Math.PI / 2;
				this.scene.frontCamera.rotation.z = this.scene.cameraP.rotation.z;
				if (this.scene.frontCamera.newZoom == 0) {
					this.scene.frontCamera.position.z = camera.position.z - 2;
				}
				// this.scene.topCamera.rotation.y = 0.2;
				
				// this.scene.frontCamera.position.z = 17.45262388584732;//pitch
				// this.scene.frontCamera.rotation.x = Math.PI / 2 + this.scene.view.pitch;
				// this.scene.frontCamera.rotation.z = this.scene.view.yaw;
			// }
...
Potree.PlanarControls = class PlanarControls extends THREE.EventDispatcher {
	constructor (viewer) {
		super(viewer);
		...
		let drag = (e) => {
			...
			// this.scene.topCamera.position.copy(camera.position);
			// this.scene.sideCamera.position.copy(camera.position);
			// this.scene.frontCamera.position.copy(camera.position);
	...
	update (delta) {
		...
		if (this.wheelDelta !== 0) {
			...
			if (I) {
				...
				camera.newZoom = camera.position.z;
				// {
				// 	let distance = resolvedPos.distanceTo(I.location);
				// 	view.radius = distance;
				// 	let speed = view.radius / 2.5;
...
Potree.PointCloudOctree = class extends Potree.PointCloudTree {
	...
	pick(viewer, camera, ray, params = {}){
		...
		// if(camera.name == 'topCamera' || camera.name == 'sideCamera' || camera.name == 'frontCamera') {
		// 	params.x = params.x - size.width / 2;
		// 	if (camera.name == 'sideCamera') {
		// 		params.y = (size.height - params.y) - (size.height / 3 + 60);
		// 	}
		// }

		// if (params.click) {
			size.width = camera.width || size.width;
			size.height = camera.height || size.height;
		// }
...
/*
 * @Author: tao.w
 * @Date: 2019-05-09 17:55:17
 * @LastEditors: tao.w
 * @LastEditTime: 2019-10-23 20:48:27
 * @Description: 
 */
...
Potree.PointCloudOctreeGeometryNode = class PointCloudOctreeGeometryNode extends Potree.PointCloudTreeNode{
	...
	constructor(name, pcoGeometry, boundingBox, _isSqlite){
		super();
	...
	getURL(){
        ...
        }else if (version.equalOrHigher('1.5')) {
	...
	loadHierachyThenPoints(){
		...
		if ((node.level % node.pcoGeometry.hierarchyStepSize) === 0) {
            ...
            if(this.isSqlite){
                hurl = node.pcoGeometry.octreeDir  + node.name;
            }
...
/*
 * @Author: tao.w
 * @Date: 2019-09-05 17:20:02
 * @LastEditors: tao.w
 * @LastEditTime: 2019-09-05 17:20:02
 * @Description: 
 */
...
Potree.utils = class {
	...
	static getMousePointCloudIntersection (mouse, camera, viewer, pointclouds, params = {}, isClick = false) {
		
		let renderer = viewer.renderer;

		let width = camera.width || renderer.domElement.clientWidth,
		height = camera.height || renderer.domElement.clientHeight;

		// if (viewer.scene.isPlanarView) {
		// 	camera = Potree.utils.getCurrentCamera(mouse);
		// 	// width = camera.width;
		// 	// height = camera.height;
		// }
		...
		pickParams.y = height - mouse.y;
		pickParams.click = isClick;
	...
	static getCurrentCamera(mouse) {
			// return [this.cameraP, this.topCamera, this.sideCamera, this.frontCamera];
		let scene = viewer.scene,
		width = viewer.renderer.getSize().width,
		height = viewer.renderer.getSize().height,
		camera;

		if (mouse.x <= width / 2) {//左侧视图
			// console.log('透视视角');
			camera = scene.cameraP;
			camera.width = width / 2;
			camera.height = height;
			return camera;
		} else {//右侧视图
			if (mouse.y <= (height / 3)) {//顶部视角
				// console.log('顶部');
				camera = scene.topCamera;	
			} else if (mouse.y > (height / 3) && mouse.y <= (height / 3 + height / 3) + 26) {//侧面视角
				// console.log('侧面');
				camera = scene.sideCamera;
			} else {//正面视角
				// console.log('正面');
				camera = scene.frontCamera;
			}
			// console.log(camera.name);
			camera.width = width / 2;
			camera.height = height / 3;
			return camera;
		}
	}
...
Potree.TextSprite = class TextSprite extends THREE.Object3D{
	...
	constructor(text){
		...
		this.fontface = 'Hiragino Sans GB';
		// this.fontsize = 10;
		// this.fontsize = 14;
		this.fontsize = 14;
 ...
 * @Date: 2019-08-09 15:14:07
 * @LastEditors: tao.w
 * @LastEditTime: 2020-06-16 15:02:35
 * @Description: 
 */
...
Potree.Measure = class Measure extends THREE.Object3D {
    constructor(opt = {}) {
        ...
        this._showArea = opt.showArea != null ? opt.showArea : false;
        this._showText = opt.showText != null ? opt.showText : false;
        //TODO  标签功能应该整合成一个， 一个元素只能有一个标签， 临时处理后面需要重构
        this._text = '';
        this._closed = true;
        // 可拖拽
        this._draggable = opt.draggable != null ? opt.draggable : true;
        //允许偏差量
        this.offset = 5;

        this._showAngles = false;
        this._showHeight = opt.showHeight != null ? opt.showHeight : false;
        this._editable = true;
        this.maxMarkers = Number.MAX_SAFE_INTEGER;
        let _color = opt.color || 0x00FF00;

        this.pointColor = opt.pointColor || 0x686363;
        this._lineType = opt.lineType || 0;  //0线渲染  1管道渲染 ,默认 0
        this.sphereGeometry = new THREE.SphereGeometry(0.0001, 8, 8);
        this.color = new THREE.Color(_color);
        this.selectColor = null;// = new THREE.Color(_color);
        this.lengthUnit = { code: 'm' };
        
        this.labelColor = opt.labelColor !=null ? opt.labelColor:{
            borderColor: { r: 0, g: 0, b: 0, a: 0.8 },
            backgroundColor:{ r: 0, g: 0, b: 0, a: 0.3 },
            textColor:{ r: 180, g: 220, b: 180, a: 1.0 }
        }

        this.spheres = [];
        this.lineWidth = opt.lineWidth || 0.003;
        this.edges = [];
        this.sphereLabels = [];
        this.edgeLabels = [];
        this.angleLabels = [];
        ...
        this.mouseDownXY = [0, 0];

        this._show = true;

        this.heightEdge;
        this.heightLabel;
        if (this.showHeight) { // height stuff
            { // height line
                //线形对象
                ...
                let lineMaterial = new THREE.LineDashedMaterial({ color: _color, dashSize: 5, gapSize: 2 });


                // lineMaterial.depthTest = false;
                this.heightEdge = new THREE.Line(lineGeometry, lineMaterial);
                this.heightEdge.visible = true;
        ...
        if (this.showText) {
            ...
            // this.textLabel = new Potree.TextSprite('');
            // this.textLabel.setBorderColor({ r: 0, g: 0, b: 0, a: 0.8 });
            // this.textLabel.setBackgroundColor({ r: 0, g: 0, b: 0, a: 0.3 });
            // this.textLabel.setTextColor({ r: 180, g: 220, b: 180, a: 1.0 });
            // this.textLabel.material.depthTest = false;
            // this.textLabel.material.opacity = 1;
    ...
    createTextLabel(){
        this.textLabel = new Potree.TextSprite('');
        this.textLabel.setBorderColor(this.labelColor.borderColor);
        this.textLabel.setBackgroundColor(this.labelColor.backgroundColor);
        this.textLabel.setTextColor(this.labelColor.textColor);
        this.textLabel.material.depthTest = false;
        this.textLabel.material.opacity = 1;
        this.textLabel.visible = false;;
        this.add(this.textLabel);
    ...
    updateLineType() {
        ...
        for (let i = 0; i < len; i++) {
            if (this._lineType === 0) {
                ...
                // let _color = new THREE.Color(this.selectColor || this.color);
                let _color = this.color;
                lineGeometry.colors.push(_color, _color, _color);
                ...
                //--------------------------------------------/
                var curve = new THREE.CatmullRomCurve3([new THREE.Vector3(), new THREE.Vector3()]);
                var geometry = new THREE.TubeGeometry(curve, 1, 0.5, 20),
                    material = new THREE.MeshPhongMaterial({ color: this.color, emissive: new THREE.Color(0x39b22f), side: THREE.DoubleSide, wireframe: true }),
    ...
    _event(edge){
        {  
            ...
            edge.addEventListener('mouseover', function (e) {
	        	...
	        	// return;
	        	// console.log(e)
                // edge.material.color与measure.color是同一个对象，改全体颜色
                if (!measure._default_color) {
                    measure._default_color = new THREE.Color(measure.color);
                }
                measure.color =  new THREE.Color("rgb(0, 244, 191)");
                measure._render = true;
                this.dispatchEvent({
                    'type': 'measureRenderState',
                    'state': true
                });
            });
            edge.addEventListener('mouseleave', function (e) {
	        	...
	        	// console.log(e)
                if (measure._default_color) {
                    measure.color.setHex(measure._default_color.getHex());
                    measure._render = true;
                }
                this.dispatchEvent({
                    'type': 'measureRenderState',
            ...
            let mouseupCallback = (e) => {
	        	// return;
	        	// console.log(e)
				if(!e.target.parent || !e.target.parent.name) return;
				let _id = e.target.parent.name;
				disease_player && disease_player.disease_come_info(_id);
				
				return;
				
				
                if (this.measuringTool.measureName != 'add_node') return;
                let xOffset = Math.abs(e.event.clientX - this.mouseDownXY[0]);
                let yOffset = Math.abs(e.event.clientY - this.mouseDownXY[1]);
                if (Math.max(xOffset, yOffset) > this.offset) {
                    this.measuringTool.measureName = '';
                    return;
                }

                let index = this.edges.indexOf(e.target);
                if (index == -1) {
                    this.measuringTool.measureName = '';
                    return
                };
                ++index;
                let hoveredElements = this.inputHandler.hoveredElements;
                let object = hoveredElements.find(d => { return d.object == e.target });
                let point = object.point;

                let sphere = this.addMarker(point, { 'index': index });
                this.update();

                this.dispatchEvent({
                    type: 'line_marker_added',
                    measurement: this,
                    index,
                    sphere: sphere
                });
                this.measuringTool.measureName != 'add_node' && (this.measuringTool.measureName = '');
            }
            ...
            let mousedownCallback = (e) => {
                ...
                this.mouseDownXY = [0, 0];
                if (e.button === THREE.MOUSE.LEFT) {
                    this.mouseDownXY = [e.event.clientX, e.event.clientY];
                }
            }

            edge.addEventListener('mouseup', mouseupCallback);
            edge.addEventListener('mousedown', mousedownCallback);
    ...
    createSphereMaterial() {
        let sphereMaterial = new THREE.MeshLambertMaterial({
            ...
            color: new THREE.Color(this.selectColor || this.color)
    ...
    addMarker(point, opt = {}) {
        ...
        sphere.name = opt.id || '';
        sphere.type = 'point';
        sphere.editable = this.editable;
        sphere.draggable = this.draggable;
   
        this.add(sphere);
        if (opt.index) {
            this.points.splice(opt.index, 0, point);
            this.spheres.splice(opt.index, 0, sphere);
        } else {
        ...
        //     let mouseupCallback = (e) => {
        //         if (viewer.measuringTool.measureName != 'add_node') return;
        //         let xOffset = Math.abs(e.event.clientX - this.mouseDownXY[0]);
        //         let yOffset = Math.abs(e.event.clientY - this.mouseDownXY[1]);
        //         if (Math.max(xOffset, yOffset) > this.offset) {
        //             viewer.measuringTool.measureName = '';
        //             return;
        //         }
        ...
        //         };
        //         ++index;
        //         let hoveredElements = viewer.inputHandler.hoveredElements;
        //         let object = hoveredElements.find(d => { return d.object == e.target });
        //         let point = object.point;
        ...
        //             sphere: sphere
        //         });
        //         viewer.measuringTool.measureName != 'add_node' && (viewer.measuringTool.measureName = '');
        //     }
        ...
        if (opt.index) {
            this.setMarker(opt.index, point);
        } else {
    ...
    removeMarker(index, endIndex) {
        endIndex = isNaN(endIndex) ? index + 1 :
            endIndex < index ? index + 1 : endIndex;

        for (var i = index; i < endIndex; i++) {
            if (!this.points[index]) break;
            this.points.splice(index, 1);

            this.remove(this.spheres[i]);
            let edgeIndex = (i === 0) ? 0 : (i - 1);
    ...
    update() {
        ...
        if (this.points.length === 0) {
            return;
        } else if (this.points.length === 1 && this.coordinateLabels[0]) {
            let point = this.points[0];
            let position = point.position;
            this.spheres[0].position.copy(position);
            this.spheres[0].draggable = this.draggable;

            { // coordinate labels
                let coordinateLabel = this.coordinateLabels[0];

                let msg = position.toArray().map(p => Potree.utils.addCommas(p.toFixed(4))).join(", ");
                //let msg = Potree.utils.addCommas(position.z.toFixed(4) + " " + this.lengthUnit.code);
        ...
        for (let i = 0; i <= lastIndex; i++) {
            ...
            sphere.material.color = new THREE.Color(this.pointColor);

            sphere.visible = this.show;
            { // edges
                let edge = this.edges[index];

                edge.material.color = new THREE.Color(this.selectColor || this.color);
                // edge.material.color.setHex((this.selectColor || edge.material.color || this.color).getHex());
                // let geo = edge.geo;
                // geo.vertices[0].set(0, 0, 0);;
                // geo.vertices[1].copy(nextPoint.position).sub(point.position);
                // edge.g.setGeometry( geo );
                ...
                if (this._lineType === 0) {
                    ...
                    let np = nextPoint.position.clone()
                    var curve = new THREE.CatmullRomCurve3([new THREE.Vector3(), np.sub(point.position)]);
                    var geometry1 = new THREE.TubeGeometry(curve, 1, this.lineWidth, 8)
                    edge.geometry = geometry1;
                }

                edge.geometry.verticesNeedUpdate = true;
                edge.geometry.computeBoundingSphere();
                edge.visible = index < lastIndex || this.closed;
                edge.visible = !this.show? this.show: index < lastIndex || this.closed;
            }

            if (this.showDistances && this.edgeLabels[i]) { // edge labels
                ...
                edgeLabel.setText(Potree.utils.addCommas(distance.toFixed(4)) + ' ' + this.lengthUnit.code);
                edgeLabel.visible = this.showDistances && (index < lastIndex || this.closed) && this.points.length >= 2 && distance > 0;
            }

            if (this.showAngles && this.angleLabels[i]) { // angle labels
                ...
                let msg = Potree.utils.addCommas((angle * (180.0 / Math.PI)).toFixed(2)) + '\u00B0';
        ...
        if (this.heightLabel && this.heightEdge) { // update height stuff
            ...
            if (this.showHeight) {
                ...
                let msg = Potree.utils.addCommas(height.toFixed(4)) + ' ' + this.lengthUnit.code;
                this.heightLabel.setText(msg);
            }
        }

        if (this.showArea && this.areaLabel) { // update area label
            this.areaLabel.position.copy(centroid);
            this.areaLabel.visible = this.showArea && this.points.length >= 3;
            let msg = Potree.utils.addCommas(this.getArea().toFixed(2)) + ' ' + this.lengthUnit.code + '\u00B2';
            this.areaLabel.setText(msg);
        }
        if (this.textLabel) { // update area label
            this.textLabel.position.copy(centroid);
            this.textLabel.visible = this.showText;
            let msg = Potree.utils.addCommas(this.text);
...
/*
 * @Author: tao.w
 * @Date: 2019-08-30 14:30:43
 * @LastEditors: tao.w
 * @LastEditTime: 2019-12-19 10:33:15
 * @Description: 
 */
...
Potree.MeasuringTool = class MeasuringTool extends THREE.EventDispatcher {
	constructor(viewer) {
        ...
        this.measureName = ''; //绘制过程中属性，用来记录绘制类型。 add_node保留字段，用来添加线面上节点
        ...
        this._maxMarkers = 0; //绘制结束时需要清空；
        this._drawMeasure = null; //新增绘制measure;
		...
		this.onAdds = e => {
            for(let i=0;i<e.measurements.length;i++){
                this.scene.add(e.measurements[i]);
            }
		};
		...
		viewer.scene.addEventListener('measurements_added', this.onAdds);
	...
	startInsertion(args = {}) {
        ...
        this._maxMarkers = measure.maxMarkers;
        
        this.measureName = measure.name;
        
        if(this._maxMarkers  == 1 && this.measureName == 'add_node'){
            return ; 
        }
		...
		domElement.removeEventListener('mouseup', this.mouseupCallback, true);
        this.mouseupCallback = (e) => {
            ...
            if(measure.name == 'add_node'){
                this.scene.remove(measure); 
            }else{
				let xOffset = Math.abs(e.clientX - this.mouseDownXY[0]);
				let yOffset = Math.abs(e.clientY - this.mouseDownXY[1]);
				let dragging = false;
                if(Math.max(xOffset, yOffset) > this.offset){
					dragging = true;
					this.scene.remove(measure); 
        ...
        this.mouseupCallback.cancel = () => {
            measure && this.scene.remove(measure); 
            this.viewer.removeEventListener('cancel_insertions', this.mouseupCallback.cancel);
            this._maxMarkers = 0;
            domElement.removeEventListener('mouseup', this.mouseupCallback, true);
            this.measureName = '';
        }
        ...
        let mousedownCallback = (e) => {
            ...
            this.mouseDownXY = [0,0];
            if (e.button === THREE.MOUSE.LEFT) {
                this.mouseDownXY = [e.clientX,e.clientY];
            }
        }
        let _cancel = (e) => {
            cancel.callback(e, true);
        }

		cancel.callback = (e,isdbclick) => {
			if (cancel.removeLastMarker) {
                if(isdbclick && measure.points.length>1){
                    measure.removeMarker(measure.points.length - 2, measure.points.length);
                }else {
                    measure.removeMarker(measure.points.length - 1);
                }
			}
            domElement.removeEventListener('mouseup', insertionCallback, true);
            domElement.removeEventListener('mousedown', mousedownCallback, true);
            if(isdbclick){
                this.viewer.dispatchEvent({
                    type: 'measure_added',
                    measure,
            ...
            this.removeEventListener('dblclick', _cancel);
            this._maxMarkers = 0;
            
            this.viewer.dispatchEvent({
                type: 'end_inserting_measurement',
                isDblClick: isdbclick == true
            });
            this._drawMeasure = measure;
		};
		...
		if (measure.maxMarkers > 1) {
            ...
            domElement.addEventListener('mousedown', mousedownCallback, true);
            this.addEventListener('dblclick', _cancel);
			// this.viewer.inputHandler.addEventListener('dblclick', _cancel, true);
			// domElement.addEventListener('dblclick', _cancel, true);
		} else {
			domElement.addEventListener('mousedown', mousedownCallback, true);
            domElement.addEventListener('mouseup', this.mouseupCallback, true);
            this.viewer.addEventListener('cancel_insertions', this.mouseupCallback.cancel);
		}
        ...
        let mouseDown = this.viewer.inputHandler.mouseDoewnMouse;
        if(mouseDown){
            this.viewer.inputHandler.onMouseMove(new MouseEvent('mousemove', {
                button: 0,
                buttons: 0,
                clientX: mouseDown.x,
                clientY: mouseDown.y,
	...
	update(){
        ...
        if(this._drawMeasure){
            this._drawMeasure._render = true;
        }
        ...
        for (let measure of measurements) {
			...
			// 	}
                ...
                //     // console.log(lowEl,startEL,endEL,lToS,sToE);
...
/*
 * @Author: tao.w
 * @Date: 2019-09-06 14:22:24
 * @LastEditors: tao.w
 * @LastEditTime: 2019-12-19 20:14:01
 * @Description: 
 */
...
class PotreeRenderer {
	...
	render(){
		...
		let r_width = viewer.renderer.getSize().width;
		let r_height = viewer.renderer.getSize().height;
		...
		let activeCam = viewer.scene.cameraP;
		if (viewer.scene.isPlanarView) {
			viewer.renderer.setViewport( 1, 1, 0.5 * r_width, r_height);
		} else {
			viewer.renderer.setViewport( 1, 1, r_width, r_height);
			viewer.scene.cameraP.width = r_width; 
			viewer.scene.cameraP.height = r_height; 
		}
 ...
 * @Date: 2018-03-12 03:22:02
 * @LastEditors: tao.w
 * @LastEditTime: 2019-08-23 18:06:27
 * @Description: 
 */
...
class EDLRenderer{
	...
	render(){
		...
		viewer.renderer.setViewport( 1, 1, 0.5 * width, height);
...
/*
 * @Author: tao.w
 * @Date: 2019-06-19 18:31:08
 * @LastEditors: tao.w
 * @LastEditTime: 2019-08-27 17:26:59
 * @Description: 
 */
 ...
 * @Date: 2019-09-05 17:20:02
 * @LastEditors: tao.w
 * @LastEditTime: 2019-09-19 15:26:40
 * @Description: 
 */
...
Potree.Scene = class extends THREE.EventDispatcher {
    ...
    constructor() {
        ...
        this.isPlanarView = false;//是否开启三视图
        this.scene = new THREE.Scene();
        ...
        this.sceneFG = new THREE.Scene();
		//前景相机
        this.cameraFG = new THREE.Camera();
        ...
        //     -5000,            			// Near 
        //     10000*10000);//new THREE.PerspectiveCamera(this.fov, 1, 0.1, 1000 * 1000);
        this.topCamera = new THREE.PerspectiveCamera(this.fov, 1, 0.1, 1000 * 1000);
        this.topCamera.name = 'topCamera';
        this.topCamera.newZoom = 0;
        
        //侧视相机
        // this.sideCamera = new THREE.OrthographicCamera(
        //     SCREEN_WIDTH / -4,	SCREEN_WIDTH / 4,		
        //     SCREEN_HEIGHT / 4,	SCREEN_HEIGHT / -4,
        //     -5000,            			// Near 
        //     10000*10000);//new THREE.PerspectiveCamera(this.fov, 1, 0.1, 1000 * 1000);
        this.sideCamera = new THREE.PerspectiveCamera(this.fov, 1, 0.1, 1000 * 1000);
        this.sideCamera.name = 'sideCamera';
        this.sideCamera.newZoom = 0;
        ...
        //     -5000,            			// Near 
        //     10000*10000);
        this.frontCamera = new THREE.PerspectiveCamera(this.fov, 1, 0.1, 1000 * 1000);
        this.frontCamera.name = 'frontCamera';
        this.frontCamera.newZoom = 0;
        ...
        // this.self_shape = THREE.SceneUtils.createMultiMaterialObject( 
        //     new THREE.TorusKnotGeometry( 30, 6, 160, 10, 2, 5 ), [ colorMaterial, wireMaterial ] );

        //前景图片
        this.fg_image = new Image();
        this.fg_texture = new THREE.Texture();
        this.sceneBG.fg_texture_1 = new THREE.Texture();
        this.sceneBG.fg_material_1 = null;
        this.fg_material = null;
        this.bg_material = null;


        //背景图片
        this.bg_image = new Image();
        this.bg_texture = new THREE.Texture();
        ...
        this.currentCamera =  this.cameraMode == Potree.CameraMode.PERSPECTIVE ? this.cameraP : this.cameraO;

        this.pointclouds = [];
        this.selectColor = new THREE.Color(0xff0000);
        //坐标轴辅助
        var axes = new THREE.AxisHelper(100000000000000000000000000000);
        this.scene.add(axes);
        this.measurements = [];
        this.profiles = [];
        this.volumes = [];
        ...
        this._selectMeasurements = [];
        this._selectMeasurementMarkers = [];
    ...
    removePointClouds(){
        for(let i=0;i<this.pointclouds.length;i++){
            let _p = this.pointclouds[i];
            this.scenePointCloud.remove(_p);
        }
        
        this.pointclouds.length = 0;

        this.dispatchEvent({
            type: 'pointclouds_remove'
        });
    };
    
    removePointCloud(pointcloud) {

        let idx = this.pointclouds.indexOf(pointcloud);
        if(idx == -1) return ;
        
        this.pointclouds.splice(idx,1);
        this.scenePointCloud.remove(pointcloud)
        
        this.dispatchEvent({
            type: 'pointcloud_remove',
            pointcloud: pointcloud
        });
    }

    getPointCloud(name){
        let p = this.pointclouds.find(d=>{
    ...
    selectMeasurement(measurements) {
        ...
        if (this._selectMeasurements.length) {
            // measurements  = [];
            for (let i = 0; i < this._selectMeasurements.length; i++) {
                this._selectMeasurements[i].selectColor = null;
                this._selectMeasurements[i]._render = true;
            }
        }

        for (let i = 0; i < measurements.length; i++) {
            measurements[i].selectColor = new THREE.Color(this.selectColor);
            measurements[i]._render = true;
        }
        this._selectMeasurements = measurements;
    }
    
    selectMeasurementMarker(list) {
        if (!Array.isArray(list)) return;
        var selectedMeasures = this._selectMeasurementMarkers;
        if (selectedMeasures.length) {
            // measurements  = [];
            for (let i = 0; i < selectedMeasures.length; i++) {
                selectedMeasures[i].material.emissive.setHex(0x000000);
                if(selectedMeasures[i].parent){
                    selectedMeasures[i].parent._render = true;
                }
            }
        }
        ...
        for (let i = 0; i < list.length; i++) {
            let sphere = list[i].measure.spheres[list[i].index];
            sphere.material.emissive.setHex(this.selectColor.getHex());
            list[i].measure._render = true;
            selectedMeasures.push(sphere);
        }
        this.dispatchEvent({
            'type': 'measurement_selected',
            'scene': this,
            'measurement': selectedMeasures
        });
    }
    ...
    removeMeasurements() {
        let arr = [].concat( this.measurements );
        for (var i = 0; i <arr.length; i++) {
            let _measurement = arr[i];
            this.removeMeasurement(_measurement);
        }
        this.measurements.length = 0;
    }
    /**
     * 根据name字段删除measur,现在假设name是唯一
     * @param {*} name 
     */
    removeMeasurementByName(name) {
        let resutl;
        for (var i = 0; i < this.measurements.length; i++) {
            let _measurement = this.measurements[i];
            if (_measurement.name == name) {
                resutl = _measurement;
                break;
            }
        }
        if (resutl) {
            this.removeMeasurement(resutl);
        }
    }
    ...
    //只返回第一个匹配到名称的
    getMeasurementByName(name) {
        let o = this.measurements.find(d => {
            return d.name == name
        })
        return o;
    }
    ...
    initialize() {
        ...
        { // background
            ...
            this.sceneBG.fg_texture_1.image = this.bg_image;
            this.sceneBG.fg_texture_1.minFilter = THREE.LinearFilter;
            this.bg_image = function () {
                this.sceneBG.fg_texture_1.needsUpdate = true;
            };

            //前景材质
            this.sceneBG.fg_material_1 = new THREE.MeshBasicMaterial({
                map: this.sceneBG.fg_texture_1,
                transparent: true,
                opacity: 1
        ...
        {
			//前景纹理
            this.fg_texture.image = this.fg_image;
            this.fg_texture.minFilter = THREE.LinearFilter;
            this.fg_image = function () {
                this.fg_texture.needsUpdate = true;
                this.sceneBG.fg_texture_1.needsUpdate = true;
            };
            this.bg_texture.image = this.fg_image;
            this.fg_texture.minFilter = THREE.LinearFilter;
            this.fg_image = function () {
                this.fg_texture.needsUpdate = true;
                this.sceneBG.fg_texture_1.needsUpdate = true;
            ...
            //前景材质
            this.fg_material = new THREE.MeshBasicMaterial({
                map: this.fg_texture,
                transparent: true,
                opacity: 0
            ...
            //前景MESH
            let bg_mesh = new THREE.Mesh(
                new THREE.PlaneGeometry(2, 2, 0),
                this.fg_material
            );
            bg_mesh.needsUpdate = true;
            bg_mesh.material.depthTest = false;
            bg_mesh.material.depthWrite = false;
            //
            //this.sceneFG.add(this.cameraFG);
            this.sceneFG.add(bg_mesh);
            ...
            // this.scene.add( this.self_shape );
		}

        { 
               //背景纹理
               this.bg_texture.image = this.bg_image;
               this.bg_texture.minFilter = THREE.LinearFilter;
               this.bg_image = function () {
                   this.fg_texture.needsUpdate = true;
                   this.sceneBG.fg_texture_1.needsUpdate = true;
               };
...
/*
 * @Author: tao.w
 * @Date: 2019-09-05 17:20:47
 * @LastEditors: tao.w
 * @LastEditTime: 2020-05-15 15:23:13
 * @Description: 
 */
...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher {
    ...
    constructor(domElement, args = {}) {
        ...
        try {
            ...
            this.classifications = {
                ...
                12: { visible: true, name: 'dynamic object' },
                13: { visible: true, name: 'GROUND' },
                14: { visible: true, name: 'ROAD' }
            };
...

            {
                ...
                this.addEventListener('measureRenderState', (e) => {
                    this.isMeasureRender = e.state;
                })

                this.scene.addEventListener("volume_removed", onVolumeRemoved);
                this.scene.addEventListener('pointcloud_added', onPointcloudAdded);

                let onMeasurementAdd = (e) => {
                    this.isMeasureRender = true;
                }

                this.scene.addEventListener('measurement_added', onMeasurementAdd);
                this.scene.addEventListener('measurements_added', onMeasurementAdd);

                this.scene.addEventListener('measurement_removed', onMeasurementAdd);
                this.scene.addEventListener('measurement_selected', onMeasurementAdd);
            }
    ...
    setImageUrl(url) {
        ...
        loader.crossOrigin = '';
        this.image_url = url;
        //console.debug("prev set image url="+url);
        //
        ...
        loader.load(
            ...
            function (image) {
                ...
                if (obj.scene != null && obj.scene.fg_material != null) {
                    obj.scene.fg_texture.image = image;
                    obj.scene.fg_texture.needsUpdate = true;
                }
            },
    ...
    setBGImageUrl(url) {
        ...
        loader.load(
            ...
            function (image) {
                ...
                if (obj.scene != null && obj.scene.bg_material != null) {
    ...
    getControls(navigationMode) {
        ...
        } else if (navigationMode === Potree.PlanarControls) {
            return this.planarControls;
        } else {
            return null;
        }
    }
    ...
    setImageOpacity(value) {
        if (this.scene.fg_material.opacity != value) {
            this.scene.fg_material.opacity = value;
        }
    };
     ...
     */
    getImageOpacity() {
        return this.scene.fg_material.opacity;
    }
    ...
    setCameraMode(mode) {
        ...
        this.scene.currentCamera = this.scene.cameraMode == Potree.CameraMode.PERSPECTIVE ? this.scene.cameraP : this.scene.cameraO;
    ...
    createControls() {
        ...
        { // 创建三视图控件
            this.planarControls = new Potree.PlanarControls(this);
            this.planarControls.enabled = false;
            this.planarControls.addEventListener('start', this.disableAnnotations.bind(this));
            this.planarControls.addEventListener('end', this.enableAnnotations.bind(this));
        }
    };

    toggleSidebar() {
        let renderArea = $('#potree_sidebar_container');
        let isVisible = renderArea.css('left') !== '0px';

        if (isVisible) {
            renderArea.css('left', '0px');
        } else {
            renderArea.css('left', '-360px');
        }
    };
    ...
    updateAnnotations() {
        ...
        // this.scene.topCamera.updateMatrixWorld();
        // this.scene.frontCamera.updateMatrixWorld();
        // this.scene.sideCamera.updateMatrixWorld();

        let distances = [];

        let renderAreaWidth = this.renderer.getSize().width;
        if (this.scene.isPlanarView) {
            renderAreaWidth = this.renderer.getSize().width / 2;
        }
    ...
    update(delta, timestamp) {
        ...
        if (this.controls !== null) {
            // console.log("当前相机：",this.controls)
            this.controls.setScene(scene);
            this.controls.update(delta);

            // this.scene.self_shape.position.copy(scene.view.position);
			/*
			...
			this.scene.topCamera.position.copy(scene.view.position);
			this.scene.topCamera.rotation.order = "ZXY";
			// this.scene.topCamera.up = new THREE.Vector3(-1,0,-1);
			this.scene.topCamera.rotation.x = this.scene.view.pitch + 1;
			// this.scene.topCamera.rotation.y = 1;
			// this.scene.topCamera.rotation.z = 9;
			// this.scene.topCamera.lookAt( scene.view.position );
			this.scene.topCamera.rotation.z = this.scene.view.yaw;
			this.scene.topCamera.zoom = 25;

			this.scene.sideCamera.position.copy(scene.view.position);
			// this.scene.sideCamera.lookAt( scene.view.position );
			this.scene.sideCamera.rotation.order = "ZXY";
			this.scene.sideCamera.rotation.x = Math.PI / 2 + this.scene.view.pitch;
			// this.scene.sideCamera.up = new THREE.Vector3(-1,0,1);
			// viewer.scene.sideCamera.lookAt(new THREE.Vector3(1,-10,0) );
			// this.scene.sideCamera.lookAt(scene.view.position);
			// this.scene.sideCamera.rotation.z = -this.scene.topCamera.rotation.z;
			this.scene.sideCamera.zoom = 70;

			this.scene.frontCamera.position.copy(scene.view.position);
			// this.scene.frontCamera.lookAt( scene.view.position );
			this.scene.frontCamera.rotation.order = "ZXY";
			this.scene.frontCamera.rotation.x = Math.PI / 2 + this.scene.view.pitch;
			this.scene.frontCamera.rotation.z = this.scene.view.yaw;
			// this.scene.frontCamera.zoom = 70;
			*/
        }
        ...
        if (!this.freeze) {
            ...
            if (this.scene.cameraMode == Potree.CameraMode.ORTHOGRAPHIC && this.IR) {
    ...
    render() {
        ...
        { // resize
            ...
            // this.scene.topCamera.aspect = aspect;
            // this.scene.topCamera.updateProjectionMatrix();

            // this.scene.sideCamera.aspect = aspect;
            // this.scene.sideCamera.updateProjectionMatrix();

            // this.scene.frontCamera.aspect = aspect;
            // this.scene.frontCamera.updateProjectionMatrix();
        ...
        let isRender = false;
        if (this.cameraChange || this.isMeasureRender || Potree.numNodesLoading != 0
            || this.measuringTool.measureName || this.dragging || this.poiCloudRender) {
            isRender = true;
            this.poiCloudRender = false;
        }
        try {

            var r_width = this.renderer.getSize().width;
            var r_height = this.renderer.getSize().height;
            let topCamera = this.scene.topCamera;
            let cameraLeft = this.scene.sideCamera;
            let cameraFront = this.scene.frontCamera;
			isRender = true;
            if (isRender) {
                if (this.useRep) {
                    if (!this.repRenderer) {
            ...
            if (this.scene.isPlanarView && isRender ) {
                /////////////顶部视角////////////
                this.renderer.setViewport(0.5 * r_width + 1, 0.6 * r_height + 75, 0.5 * r_width - 2, 0.3 * r_height);
                this.pRenderer.render(this.scene.scenePointCloud, topCamera);
                this.renderer.render(this.scene.scene, topCamera);
                // console.log("顶部:", 0.5 * r_width - 2,  0.3 * r_height - 2);
                // ////////////左侧视角////////////
                this.renderer.setViewport(0.5 * r_width + 1, 0.3 * r_height + 50, 0.5 * r_width - 2, 0.3 * r_height);
                this.pRenderer.render(this.scene.scenePointCloud, cameraLeft);
                this.renderer.render(this.scene.scene, cameraLeft);
                // console.log("左侧:", 0.5 * r_width - 2,  0.3 * r_height - 2);
                // ////////////正视视角////////////
                this.renderer.setViewport(0.5 * r_width + 1, 1, 0.5 * r_width - 2, 0.3 * r_height - 2);
                this.pRenderer.render(this.scene.scenePointCloud, cameraFront);
                this.renderer.render(this.scene.scene, cameraFront);
                // console.log("正视:", 0.5 * r_width - 2,  0.3 * r_height - 2);
            }

//          if (viewer.image_url != undefined && viewer.image_url != '' && isRender) {
//              viewer.renderer.render(viewer.scene.sceneFG, viewer.scene.cameraFG);
//          }

            if (isRender || this.isMeasureRender || this.dragging) {
...
/*
 * @Author: tao.w
 * @Date: 2019-09-16 14:10:47
 * @LastEditors: tao.w
 * @LastEditTime: 2020-05-22 10:49:57
 * @Description: 
 */
initSidebar = (viewer) => {
    ...
    // viewer.measuringTool = measuringTool;
	...
	function initScene(){
		...
		let onMeasurementRemoved = (e) => {
			//  if ($("#jstree_scene").jstree().get_json) {
			// 	let measurementsRoot = $("#jstree_scene").jstree().get_json("measurements");
	...
	function initClassificationList () {
		...
		addClassificationItem(12, 'dynamic object');
		addClassificationItem(13, 'GROUND');
		addClassificationItem(14, 'ROAD');
	}
...
//		}
	...
	function initAppearance () {
		...
		$('#sldImageOpacity').slider({
			value: this.viewer.getImageOpacity(),
			min: 0,
			max: 1,
			step: 0.1,
			slide: (event, ui) => { this.viewer.setImageOpacity(ui.value); }
		});
		...
		$('#lblPointBudget')[0].innerHTML = Potree.utils.addCommas(viewer.getPointBudget());
		$('#lblFOV')[0].innerHTML = parseInt(viewer.getFOV());
		$('#lblImageOpacity')[0].innerHTML = this.viewer.getImageOpacity();
		$('#lblEDLRadius')[0].innerHTML = viewer.getEDLRadius().toFixed(1);
		$('#lblEDLStrength')[0].innerHTML = viewer.getEDLStrength().toFixed(1);
