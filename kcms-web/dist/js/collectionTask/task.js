...
var collectionTask = {
    ...
    leftTabModal_cof: {
        grp_1: [
            ...
            { name: '所属项目ID', code: 'groupProjectId' }],
        grp_2: [
            { name: '任务里程', code: 'totalMileage' },
            { name: '下发日期', code: 'assignTimeStr' },
            { name: '完成日期', code: 'endTimeStr' },
            { name: '采集设备', code: 'collectDevice' },
            { name: '采集人员', code: 'collectBy' }],
        grp_3: [
            { name: '任务来源', code: 'source' },
    ...
    init: function () {
        console.log(this);
        collectionTask.widgetDiv();
        collectionTask.mainTableDiv();
        $.mapType = collectionTask;
    ...
    widgetDiv: function (cof) {
        cof = [
            ...
            { id: "type", name: "任务类型", show: true, required: false, type: "select", data: [
                {
                    "name": "地图量产",
                    "value": "0"
                },
        ...
        /**条件筛选定制 */
        let _html = `<p class='page_title'><span></span>采集任务管理</p>`
    ...
    mainTableDiv: function (conditions = {}) {
        ...
        let restext = '';
        let zh_match = collectionTask.zh_match;
        let clsMatch =  collectionTask.clsMatch;
        var table = $("#project_task_1").DataTable({
            ...
            ajax: function (data, callback, settings) {
                ...
                $.postAjax(params, function (data) {
                    ...
                    collectionTask.projArr = returnData1.data || [];
                })
            },
            columns: [
                {
                ...
                {
					"data": "id" || "",
					"class": "collectTimeStr",
					"render": function (data, type, row, meta) {
						let res = restext;
						let k = 'collectTimeStr';
						let pro = row.properties || {};
                        res = row[k] || pro[k] || '';
						let div = `<span title='${res}'>${res}</span>`
						return div;
                ...
                {
					"data": "id" || "",
					"class": "groupProjectId",
					"render": function (data, type, row, meta) {
						let res = restext;
						let k = 'groupProjectId';
						let pro = row.properties || {};
                        res = row[k] || pro[k] || '';
						let div = `<span title='${res}'>${res}</span>`
						return div;
    ...
    newItemDiv: function (_projData = {}) {
        ...
        Object.entries(collectionTask.projJson).forEach(([k, v]) => {
        ...
        var formBody = `            
                <div class="createForms">
                    ...
                    <div class="col-md-12">
                        <div class="dialog-header projtype">采集类型</div>
                        ${domJson.projtype.map(el => {
            let _active = (el.code === _projJson.projtype ? 'active' : '');
            return `<button class="dialog-btn grpBtn ${_active} ${el.code}" name='${el.code}'>${el.name}</button>`
        }).join('')}
            ...
            return `<select class="widget_ipt_sub_xs ${el.code}" name='${el.code}'>${el.data.map(ell => {
                ...
                return `<option ${_select} class="${ell.code}" name='${ell.code}'>${ell.name}</option>`
                    ...
                    <div class="col-md-12" style='margin-bottom:20px;'>
                        ...
                        <textarea class="dialog-text">${_projJson.remark}</textarea>
                    </div>
                    <div class="col-md-12">
                    <div class="col-md-12 sub-col">
                        ...
                        <button class="dialog-btn primaryBtn surveyAreaDraw"  data-num='0'  style="vertical-align: middle;">绘制区域</button>
                            <button class="addRect widget_cube_btn_sub_xs"  data-num='0' style="background: #5E66F8;color:#fff;vertical-align: middle;">
                            <img class='sketch-icon'  src="../bower_components/sketch-icon/plus_circle.svg">
                            </button>
                    </div>
                    </div>
                    <div class="col-md-12">
                        <div class="dialog-header">备注</div>
        ...
        let mapCof = {
            ...
            infoOpen: 'infoOpen',
            _callback: collectionTask.afterNewItem,
            type: _head,
        ...
        $.modalCreate_new({
            ...
            head: _head,
            _fn: [{
                fn: collectionTask.mapInit,
                param: mapCof,
            }],
        }, function () {//submit
            collectionTask.submitRegion();
        })
    },
    /**新增项目弹窗的监听事件 */
    afterNewItem: function (param) {
        //格式化
        collectionTask.polygonArr = {};
        collectionTask.projLayers = {};
        collectionTask.drawNum = '';
        //绘制区域

        if (param.type == '编辑采集项目' && param.projectId && param.surveyArea) {
            let _ranges = param.surveyArea.range || [];

            collectionTask.drawPolygon({ ranges: _ranges, mapName: param.leafletMap }, true);
        }
        //项目名称-选择城市行政区划
        ...
        $('button.projNameSwitch').on('click', function () {
            ...
            $('button.addRect').on('click', function () {
                ...
                collectionTask.addOneArea(pardom)
        ...
        $('.testmodal .createForms').on('click', 'button.delRect', function () {
            ...
            let _polyId = collectionTask.polygonArr['poly' + _num];
            $.mapRemove(_polyId);
            delete (collectionTask.polygonArr['poly' + _num]);
            delete (collectionTask.projLayers[_polyId]);
        })
        //绘制区域
        $('.createForms').on('click', '.surveyAreaDraw', function () {
            ...
            let _num = $(this).attr('data-num');
            if (('poly' + _num) in collectionTask.polygonArr && !confirm('是否重绘区域？')) {
                return;
            } else {
                let _polyId = collectionTask.polygonArr['poly' + _num];
                $.mapRemove(_polyId);
        ...
        $('.createForms').on("mouseenter mouseleave", ".surveyAreaDraw", function (event) {
            ...
            if (_polyId && event.type == "mouseenter") {
                leafletMap._layers[_polyId].setStyle({ color: '#00bb00' });
            } else if (_polyId && event.type == "mouseleave") {
                leafletMap._layers[_polyId].setStyle({ color: '#3388ff' });
            };
        });
    ...
    //详情窗口-项目 左一列表隐藏
    itemInfoDiv: function (taskId) {
        let res = collectionTask.projArr.filter(el =>{return taskId == el.id})[0] || {};
        let _leftMenu = `<h5 class='title'>任务列表</h5><ul class="sub-sidebar-menu">${collectionTask.projArr.map(el => {
            return `<li name='${el.id || ''}'>
        <span>${el.projectName || el.name || ''}</span>
    </li>`}).join('')}     
    </ul>`;
        collectionTask.projArr.forEach(el => {
            if (el.id == taskId) {
                res = el;
            }
        })
        let data = collectionTask.leftTabModal_cof;
        let _legend =  collectionTask.legend;
        let zh_match = collectionTask.zh_match;
        var formBody = `
        ...
        ${Object.keys(data).map(subdata => {
            return data[subdata].map(el => {
                ...
                return `<div class='subTable'><div class="subheader">${el.name}</div>
                <div class="subbody ${_cls}  ${_cls_proc}">${(zh_match[el.code] && zh_match[el.code][res[el.code]]&&zh_match[el.code][res[el.code]].name) || res[el.code] || ''}</div></div>`}).join('')
        }).join('<br>')}</div></div><div id="leafletMap_info" class="modal-body"></div>`;
        let mapCof = {
            leafletMap: 'leafletMap_info',
            infoOpen: 'infoOpen_info',
            _callback: collectionTask.afterItemInfo,
            groupProjectId: res.id || '',
            surveyArea: res.surveyArea || {}

        };

        let chartPara = {};
        Object.assign(chartPara,collectionTask.chartPara);
        ...
        let init_cof = {
            ...
            _leftHtml: _leftMenu,
            leftListener: {
                fn: collectionTask.evtListen,
                param: 'iteminfo'
            }
        };

        $.modalCreate_new(init_cof, function () { })
    },
    /** */
    afterItemInfo: function (param) {
        let _ranges = param.surveyArea.ranges || [];
        collectionTask.drawPolygon({ ranges: _ranges, mapName: param.leafletMap });
    },
    //II 
    //widget栏格式化
    ...
    editProj: function (projectId) {
        ...
        collectionTask.newItemDiv(_projData);
    },
    //绘制polygon
    drawPolygon: function (data, readonly = false) {
        ...
        data.ranges.forEach((el, i) => {
            ...
            collectionTask.polygonArr['poly' + i] = leafletId;
            collectionTask.projLayers[leafletId] = {
                'range': el
            };
            if (readonly) {
    ...
    drawPolyline: function (data, readonly = false) {
        ...
        _collectionLinks.forEach(el => {
            ...
            collectionTask.polylineArr['poly' + el.id] = leafletId;
            collectionTask.projLayers[leafletId] = el.properties||{};
        })
    ...
    rectBounds: function (layers) {
        ...
        collectionTask.polygonArr['poly' + collectionTask.drawNum] = leafletId;
    ...
    evtListen: function (type) {
        if (type === 'widgetDiv') {
            ...
            $('.primaryHeader button.queryProj').on('click', function () {
                ...
                collectionTask.mainTableDiv(conditions);
            });
            /**监听add */
            $('.primaryHeader button.addProj').on('click', function () {
                collectionTask.newItemDiv();
            })
        } else if (type === 'mainTableDiv') {
            /**监听show_detail */
            $('#project_task_1').on('click', 'a.show_detail', function () {
                let _projectId = this.name;
                collectionTask.itemInfoDiv(_projectId);
            })
            /**监听edit */
            $('#project_task_1').on('click', 'a.edit', function () {
                collectionTask.editProj(this.name);
            })
            /**监听delete */
            $('#project_task_1').on('click', 'a.delete', function () {
                let _projectId = this.name;
                collectionTask.deletePorj(_projectId);
            })
        } else if (type == 'iteminfo') {
            /**监听切换任务列表 */
            $('.detailmodal').on('click', '.dialog-leftMenu li', function () {
                let _taskId = $(this).attr('name');
                collectionTask.itemInfoRef(_taskId);
            })
        }

    },
    itemInfoRef: function (taskId) {
        let res = collectionTask.projArr.filter(el =>{return taskId == el.id})[0] || {};
        let data = collectionTask.leftTabModal_cof;
        let zh_match = collectionTask.zh_match;
        // 进度评估 缺失
   ...
   chartPara.option.series[0].data[0].name = (zh_match.status[res.status] && zh_match.status[res.status].name) || res.status;
   chartPara.option.series[0].data[0].value = res.process || '0.0';
        $('.detailmodal .leftTab').html(leftTabHtml);
        let proces_chart = echarts.init(document.getElementById(chartPara.id));
        proces_chart.clear();
        proces_chart.setOption(chartPara.option, true);
        proces_chart.resize();
        Object.entries(collectionTask.polygonArr).forEach(([id, val]) => {
            $.mapRemove_new(val, 'leafletMap_info');
        });
        // let _surveyArea = $.geoPolygon_new(res.polygons);   //接口暂时没有这个数据  bbox可能之后会渲染
        // collectionTask.drawPolygon({ ranges: _surveyArea || [], mapName: 'leafletMap_info' });
        // let _surveyLinks = $.geoPolyline_new(res.collectionLinks); //路线数组
        res['mapName'] = 'leafletMap_info';
        Object.entries(collectionTask.projLayers).forEach(([k,v]) => {
             $.mapRemove_new(k,'leafletMap_info');
        });       
        collectionTask.drawPolyline(res);
    }
