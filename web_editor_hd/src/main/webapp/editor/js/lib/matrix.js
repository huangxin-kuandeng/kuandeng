 ...
 * @param arrA
 */
matrix.homogeneous = function(arrA){
    arrA.push([1]);
}
/**
...
matrix.getRow = function(M,n){
    if (n < 0) {
        throw new Error('The specified column must be a positive integer.');
    } else if (n >= M.length) {
        throw new Error('The specified column must be between 0 and the number of row - 1.');
    }
    return M[n].slice();
}
/**
...
matrix.block = function(M,a, b, c, d){
    ...
    do { i = c - ni;
        ...
        nj = d;
        do { j = d - nj;
            elements[i][j] = M[(a+i-1)%rows][(b+j-1)%cols];
        } while (--nj);
    } while (--ni);
    // do { i = c - ni;
    //     elements[i] = [];
...
matrix.cross = function(vector,elements) {
    var A = vector;
    if (elements.length != 3 || A.length != 3) { return null; }
    var B = elements;
    return [
        (A[1] * B[2]) - (A[2] * B[1]),
        (A[2] * B[0]) - (A[0] * B[2]),
