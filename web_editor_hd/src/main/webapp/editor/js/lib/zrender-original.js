...
function Draggable() {
//  this.on('mousedown._zrender_draggable', this._dragStart, this);
    this.on('mousedown._zrender_draggable', function(e){
        var draggingTarget = e.target;
        if(draggingTarget){
        	draggingTarget.__drag_status = undefined;
        }
        if(draggingTarget && draggingTarget.draggable){
        	this._draggingTarget = draggingTarget;
        	// mousedown时不应该触发dragstart
        	// 容易与mousedown/点击(mouseup)事件冲突；
        	draggingTarget.__drag_status = 0;
        }
    }, this);
    this.on('mousemove._zrender_draggable', _drag, this);
    this.on('mouseup._zrender_draggable', _dragEnd, this);
    this.on('globalout._zrender_draggable', _dragEnd, this);
    ...
    function _drag(){
        ...
        if(draggingTarget){
			...
			if(draggingTarget.__drag_status === 0){
				// dragstart
				// 拖拽开始时， 若是移动过快 ，事件触发的时候鼠标位置没有target
				// 或是鼠标在其他的target上
				if(args[0]){
					args[0].target = draggingTarget;
				}
				this._dragStart.apply(this, args);
			}
			// draging
			draggingTarget.__drag_status = 1;
			this._drag.apply(this, args);
        }
    }
    ...
    function _dragEnd(){
        ...
        if(draggingTarget){
        	var args = Array.prototype.slice.call(arguments);
        	// dragend
        	if(draggingTarget.__drag_status === 1){
        		this._dragEnd.apply(this, args);
        	}
        	draggingTarget.__drag_status = undefined;
        }
        this._draggingTarget = undefined;
    }
...
Eventful.prototype = {
    ...
    on: function (event, handler, context) {
        ...
        // 参考命名空间，但是off时必须声明完整事件+命名空间，否则不能清空该事件；
        // 只支持一层命名空间；
        // 触发事件之后，触发命名空间的事件
		var suffix = event.split('.');
		suffix = suffix.length > 1 && suffix[suffix.length - 1] || undefined;
        _h[event].push({
            h: handler,
            one: false,
            ctx: context || this,
    ...
    trigger: function (type) {
    	var handlers = this._handlersByEvent(type);
    	if(handlers.length){
    		var args = arguments;
    ...
    /**
     * 根据事件名获取事件数组，只考虑命名空间一层
     * @param {String} type
     */
    _handlersByEvent: function(type){
    	var suffix = type.split('.');
    	var result = [];
    	if(suffix.length > 1){
    		var list = this._$handlers[type];
    		if(list) result.push(list);
    		return result;
    	}
    	var obj = this._$handlers, flist;
    	for (var key in obj) {
            if (obj.hasOwnProperty(key)) {
                let names = key.split('.');
                if(names[0] === type){
...
Handler.prototype = {
    ...
    mousemove: function (event) {
        ...
        proxy.setCursor && proxy.setCursor(hoveredTarget ? hoveredTarget.cursor : this.defaultCursor);
    ...
    if (points && points.length >= 2) {
        if (smooth && smooth !== 'spline') {
			...
			if (!shape.lineType || shape.lineType == 'solid') {
                ...
                || shape.lineType == 'dotted'
            ) {
                var dashLength = (shape.lineWidth || 1)
                    * (shape.lineType == 'dashed' ? 5 : 1);
                ctx.moveTo(points[0][0], points[0][1]);
                for (var i = 1; i < len; i++) {
                    dashedLineTo(
                        ctx,
            ...
            } else if (shape.lineType == "dashed&solid") {
            	var prevStart = false;
                for (var i = 0; i < points.length; i++) {
                    ...
                    if (i == 0) {
						...
						var lastStep = points[i-1];
                        if (lastStep[2]) {
							if (lastStep[2] == "start") {
								ctx.lineTo(point[0], point[1]);
                            } else if (lastStep[2] == "end"){
								ctx.moveTo(point[0], point[1]);
                            }else if(lastStep[2] == "middle"){
	                    		if(isVisibleMiddle(i-1)){
									ctx.lineTo(point[0], point[1]);
	                    		}else {
									ctx.moveTo(point[0], point[1]);
	                    		}
...
//							ctx.lineTo(point[0], point[1]);
        ...
        // 终点后的中间点不显示
        function isVisibleMiddle(i){
    		for(i = i - 1; i >= 0; i--){
    			var point = points[i];
    			if(!['start', 'end'].includes(point[2])) continue;
    			if(point[2] == 'start'){
    				return true;
    			}else {
    				break;
