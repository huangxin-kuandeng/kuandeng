/*
 * @Author: tao.w
 * @Date: 2019-09-03 10:54:54
 * @LastEditors: tao.w
 * @LastEditTime: 2020-05-21 17:05:52
 * @Description: 
 */
(function (global, factory) {
	...
	function checkBufferGeometryIntersection( object, raycaster, ray, position, uv, a, b, c ) {
		...
		// let lla = iD.util.UTMtoLL_(vA.x, vA.y, 50); 
		// let llb = iD.util.UTMtoLL_(vB.x, vB.y, 50); 
		// let llc = iD.util.UTMtoLL_(vC.x, vC.y, 50); 
        // nodes.push(new Point({ loc : [lla[0], lla[1], vA.z] }));
        // nodes.push(new Point({ loc : [llb[0], llb[1], vB.z] }));
		// nodes.push(new Point({ loc : [llc[0], llc[1], vC.z] }));
		// nodes.push(new Point({ loc : [lla[0], lla[1], vA.z] }));
		...
		_buildTree( geo, options ) {
			...
			const splitNode = ( node, offset, count, depth = 0 ) => {
				...
				if ( splitOffset === offset || splitOffset === offset + count ) {
					...
					left.boundingData = ctx.getBounds( lstart, lcount, new Float64Array( 6 ) );
					splitNode( left, lstart, lcount, depth + 1 );

					// repeat for right
					const right = node.right = new MeshBVHNode();
					const rstart = splitOffset, rcount = count - lcount;
					right.boundingData = ctx.getBounds( rstart, rcount, new Float64Array( 6 ) );
			...
			if ( ranges.length === 1 ) {
				...
				if ( geo.boundingBox != null ) {
					...
					root.boundingData = ctx.getBounds( range.offset, range.count, new Float64Array( 6 ) );
