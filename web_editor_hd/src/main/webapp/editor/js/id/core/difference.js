...
iD.Difference = function(base, head) {
    ...
    function addChildren(entity,result,head)
    {
        var members=entity.members || [];
        for(var i=0;i<members.length;i++)
        {
            if(members[i].id in result)
            {
                continue;
            }
            if(head.hasEntity(members[i].id))
            {
                var memberObj=head.entity(members[i].id);
                result[memberObj.id] = memberObj;
                if(memberObj.type=="relation"){
                    addChildren(memberObj,result,head)
                }
            }
        }
    ...
    difference.complete = function(extent) {
        ...
        for (id in changes) {
            ...
            if(entity.tags && entity.modelName && addChildRenFlagSet[entity.modelName])
            {
                addChildren(entity,result,head)
    ...
    var addChildRenFlagSet={    };
    addChildRenFlagSet[iD.data.DataType.R_DIVIDER_DREF]=1;
    addChildRenFlagSet[iD.data.DataType.DIVIDER_ATTRIBUTE]=1;
    addChildRenFlagSet[iD.data.DataType.R_ROAD_DIVIDER]=1;
    addChildRenFlagSet[iD.data.DataType.R_DIVIDER_JUNCTION]=1;
