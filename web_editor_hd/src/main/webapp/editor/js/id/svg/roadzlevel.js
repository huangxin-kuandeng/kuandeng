iD.svg.RoadZLevel = function(projection, context) {
    ...
    function drawRoadZLevel(surface, points, filter) {
        points.sort(sortY);

        var groups = surface.select('.layer-zlevel').selectAll('g.point')
            .filter(filter)
	...
	drawRoadZLevel.available = function (points) {
    ...
    drawRoadZLevel.roadzlevel = function(entities, limit) {
