 ...
 */
iD.svg.SearchPoint = function(projection, context) {
        ...
        if (klass === 'shadow' || klass === 'stroke') {
            ...
            selection
                ...
                .attr('xlink:href', function(d){
                    if (d.isSearchPoint()) return context.imagePath('roadcross.png');
                });
        }
    ...
    function drawSearchPoint(surface, points, filter) {
        points.sort(sortY);
        // surface.js中添加searchpoint的layer div
        var groups = surface.select('.layer-searchpoint').selectAll('g.point')
            .filter(filter)
        ...
        */
        ...
        groups.classed('point-hidden',function(d){
            var modelConfig = iD.Layers.getLayer(d.layerId);
    ...
    drawSearchPoint.available = function (points) {
        /*
    ...
    drawSearchPoint.searchpoint = function(entities, limit) {
        ...
        for (var i = 0; i < entities.length; i++) {
            ...
            if (entity.isSearchPoint && entity.isSearchPoint()) {
                points.push(entity);
