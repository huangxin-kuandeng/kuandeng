 ...
 * Created by wt.
 */
iD.behavior.DrawBoardArea = function(context, wayId, index, mode, baseGraph) {
...
//      start = iD.Node({loc: context.graph().entity(way.nodes[startIndex]).loc,modelName: iD.data.DataType.TRAFFICSIGN_NODE}),
//      end = iD.Node({loc: context.map().mouseCoordinates(),modelName: iD.data.DataType.TRAFFICSIGN_NODE}),
        ...
        // start= iD.Node({loc: context.map().mouseCoordinates(),modelName: iD.data.DataType.TRAFFICSIGN_NODE}),
...
//      var last = context.hasEntity(way.nodes[way.nodes.length - (isArea ? 2 : 1)]);
        ...
        var wlay = iD.Layers.getCurrentModelEnableLayer(iD.data.DataType.TRAFFICSIGN_NODE);
        var wayLayerId = wlay.id;
        var newNode = iD.Node({loc: loc,
            layerId:wayLayerId,
            identifier:wlay.identifier,
