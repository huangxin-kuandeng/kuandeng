 ...
 */
iD.behavior.DrawRoadAttribute = function(context) {
    ...
    function click() {
        var d = datum(), modelConfig = iD.Layers.getLayer(d.layerId, d.modelName), isEnable = modelConfig ? modelConfig.editable : '';
        if (d.modelName != iD.data.DataType.ROAD) {
            Dialog.alert('必须点击在道路上');
            return ;
        }
        ...
        if (d.type === 'way' && isEnable) {
            ...
            event.clickWay(choice.loc, d,edge);
        }else {
