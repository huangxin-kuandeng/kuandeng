 ...
 */
iD.geo.Maat = function(turn) {
    if (!(this instanceof iD.geo.Maat))
        return new iD.geo.Maat(turn);
    _.extend(this, turn);
};

iD.geo.maatIntersection = function(graph, vertexId) {
    ...
    graph.parentWays(vertex).forEach(function(way) {
        ...
        if (way.isArea() || way.isDegenerate())
            return;
        ...
        //         wayA = iD.Way({id: way.id + '-a', modelName:way.modelName, tags: way.tags, nodes: way.nodes.slice(0, idx + 1)}),
        //         wayB = iD.Way({id: way.id + '-b', modelName:way.modelName, tags: way.tags, nodes: way.nodes.slice(idx)});
        //
    ...
    intersection.turns = function(fromNodeID) {
        ...
        function withRestriction(turn) {
            graph.parentRelations(graph.entity(turn.from.way)).forEach(function(relation) {
                ...
                var f = relation.memberByRole(iD.data.RoleType.FROAD_ID),
                    t = relation.memberByRole(iD.data.RoleType.TROAD_ID),
                    v = relation.memberByRole(iD.data.RoleType.ROAD_NODE_ID);
