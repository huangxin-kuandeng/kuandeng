iD.ui.intro = function(context) {
    ...
    function intro(selection) {
        ...
        // Block saving
        var savebutton = d3.select('#KDSEditor-bar button.save'),
        ...
        steps[steps.length - 1].on('startEditing', function() {
            ...
            d3.select('#KDSEditor-bar button.save').on('click', save);
        });

        var navwrap = selection.append('div')
		             .attr('class', 'intro-nav-wrap KDSEditor-fillD');
        ...
        entered.append('div').attr('class','KDSEditor-icon KDSEditor-icon-pre-text apply');
