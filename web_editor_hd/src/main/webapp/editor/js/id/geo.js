...
iD.geo.angleCoords = function(angle,size){
    var x , y;
    if(angle <= 0)
    {
        angle = 360 + angle;
    ...
    if(angle < 90 && angle >= 0)
    {
        x = 1 * size * Math.abs(Math.sin((Math.PI/180) * angle));
        y = -1 * size * Math.abs(Math.cos((Math.PI/180) * angle));
 ...
 * @param {Array<iD.Node>} nodes
 * @param {Object} point 像素坐标
...
iD.geo.polygonIntersectsPolygon = function(outer, inner, checkSegments) {//更新为id 最新版本代码
