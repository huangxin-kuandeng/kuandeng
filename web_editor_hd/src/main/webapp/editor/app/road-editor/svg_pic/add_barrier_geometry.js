...
// 激光线
iD.svg.PicDraw.addBarrierGeometry = function(context, drawTool){
	...
	_.assign(drawPlus.prototype, {
		init: function(arg1){
			...
			var self = this;
			player.on('picUpdate.add-barrier-geometry', function(){
		...
		getId: function(){
			return constant.ADD_BARRIER_GEOMETRY;
		},
		renderBtn: function(selection){
            selection.append('button')
                .attr('type','button')
                .attr('modelName',iD.data.DataType.BARRIER_GEOMETRY)
                .text('BARRIER')
                .data([this.getId()]);
		},
		// 绘制结束后重置变量
		...
		*/
        ...
        buttonCancel: function(evt, node){
            ...
            if(!shape || !shape._entityid) return ;
            var entity = context.hasEntity(shape._entityid);
            if(!entity || entity.modelName != iD.data.DataType.BARRIER_GEOMETRY) return ;
            iD.logger.editElement({
                'tag': "add_barrier_geometry_end",
                'entityId':entity.osmId() || '',
                'modelName': entity.modelName,
                'type': "dblClick"
            });
		...
		domClick: function(evt, clickOffset, opts = {}){
            ...
            context.buriedStatistics().merge(1,iD.data.DataType.BARRIER_GEOMETRY);
            // 点击在首尾节点上
		...
		domDblclick: function(evt, clickOffset, opts = {}){
            ...
            let isdpeth = d3.select('#picplayer-tools-bar .playerDepth').node()?d3.select('#picplayer-tools-bar .playerDepth').property('checked') : false;
            var shape = drawStatus.shape;
            var way = context.hasEntity(shape._entityid);
            if(presult._entityid){
                var geometry = iD.picUtil.pixelToLngLat(clickOffset,player.pic_point,isdpeth);
	    ...
	    clickAddDivider: function (shape, clickOffset, isDbclick = false, opts = {}){
            ...
            if(added){
                if(isNew){
                    log_obj = {
                        'tag': 'add_barrier_geometry_start', 
                        'entityId':_w.osmId() || '',
	...
	function drawGeometry2Map(drawMark, geoData, opts,self){
        var result = iD.svg.PicDrawUtil.drawLine2Map(drawMark, geoData, {
        	...
        	isCommonPoint: true,
        	nodeModelName: iD.data.DataType.BARRIER_GEOMETRY_NODE,
        	lineModelName: iD.data.DataType.BARRIER_GEOMETRY
        });
