 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2020-04-20 15:07:13
 * @Description: 
 */

iD.svg.PicDraw = iD.svg.PicDraw || {};
// 病害面
iD.svg.PicDraw.addPavementDistressLine = function (context, drawTool) {
    ...
    _.assign(drawPlus.prototype, {
        init: function (arg1) {
            ...
            var self = this;
            player.on('picUpdate.add-pavementdistressline', function () {
        ...
        getId: function () {
            return constant.ADD_PAVEMENT_DISTRESS_LINE;
        },
		...
		*/
        ...
        endDraw: function (way) {
            ...
            let buffered = turf.buffer(point, 3, { units: 'millimeters' });
            ...
            let z = nodeLocs[0][2];
            newLocs.forEach((d, i) => {
                // let _t = iD.util.UTMtoLL_(d[0], d[1], utm.zoneNumber, utm.designator);
                // let loc = [_t[0], _t[1], d[2]];
        ...
        domDblclick: function (evt, clickOffset, opts = {}) {
            ...
            iD.picUtil.updateZrenderStyleByEntity(presult, presult._entityid);
            let isok = self.clickAddDivider(presult, clickOffset, true, downInfo && {
                downNodeid: downInfo.entityid
            });
