 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2021-04-09 17:25:30
 * @Description: 高亮定位线与lane的关联关系
 */

iD.effects = iD.effects || {};

iD.effects.LaneRPL = function (context) {

    var effect = {
        id: 'lane-rpl',
        button: 'laneRPL',
        title: t('effects.laneRPL.title'),
        iconText: 'RPL',
        description: t('effects.laneRPL.description'),
        key: 'Shift+L',
        oldEntity: null,
        enable: false,
        apply: function (context, open, selectedids) {
            ...
            if (divider.modelName == iD.data.DataType.HD_LANE || divider.modelName == iD.data.DataType.OBJECT_PG) {
    ...
    function show(divider, enable) {
        ...
        var dividerRelations = graph.parentRelations(divider, iD.data.DataType.R_PG_LANE);
        // 没有关系
    ...
    // 选中divider
    context.event.on('selected.effect-laneRPL', function (selectedEntities) {
        effect.enable && effect.apply.call(effect, context, effect.enable, _.pluck(selectedEntities, 'id'));
    });

    context.map().on('drawn.effect-laneRPL', function (param) {
