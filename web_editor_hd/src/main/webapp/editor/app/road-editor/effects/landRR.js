 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2021-04-09 17:29:15
 * @Description: 双向道路关系
 */

iD.effects = iD.effects || {};

iD.effects.LaneRR = function (context) {

    var effect = {
        id: 'lane-rr',
        button: 'laneRR',
        title: t('effects.laneRR.title'),
        iconText: 'RR',
        description: t('effects.laneRR.description'),
        key: 'Shift+L',
        oldEntity: null,
        enable: false,
        apply: function (context, open, selectedids) {
            ...
            if (divider.modelName == iD.data.DataType.ROAD) {
    ...
    function show(divider, enable) {
        ...
        var dividerRelations = graph.parentRelations(divider, iD.data.DataType.R_ROAD);
        // 没有关系
    ...
    // 选中divider
    context.event.on('selected.effect-laneRR', function (selectedEntities) {
        effect.enable && effect.apply.call(effect, context, effect.enable, _.pluck(selectedEntities, 'id'));
    });

    context.map().on('drawn.effect-laneRR', function (param) {
