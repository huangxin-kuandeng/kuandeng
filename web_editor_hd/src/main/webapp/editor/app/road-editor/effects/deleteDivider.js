 ...
 */
iD.effects.deleteDivider = function(context) {
    var effect = {
        id: 'delete-divider',
        button: 'delete-divider',
        title: t('effects.deleteDivider.title'),
        iconText: 'DD',
        description: t('effects.deleteDivider.description'),
        key: 'Shift+B',
        enable: false,
...
//	        		根据是否存在关系来改变道路的颜色(存在关系为红色;不存在则为灰色)
                    if(self.enable){
                        if (entity.tags.SOURCE == '2000') {
                            effectColor = "#FF0000";
                        } else {
                            effectColor = "#A9A9A9";
                        }
