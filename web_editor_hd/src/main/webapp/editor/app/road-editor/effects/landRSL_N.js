 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2021-04-12 15:26:43
 * @Description: 高亮定位线与lane的关联关系--TYPE邻近关系
 */

iD.effects = iD.effects || {};

iD.effects.LaneRSL_N = function (context) {

    var effect = {
        id: 'lane-rsl-n',
        button: 'laneRSL_N',
        title: t('effects.laneRSL_N.title'),
        iconText: 'RSL2',
        description: t('effects.laneRSL_N.description'),
        key: 'Shift+L',
        oldEntity: null,
        enable: false,
        apply: function (context, open, selectedids) {
            ...
            if (divider.modelName == iD.data.DataType.HD_LANE || divider.modelName == iD.data.DataType.TRAFFICSIGN) {
    ...
    function show(divider, enable) {
        ...
        var dividerRelations = graph.parentRelations(divider, iD.data.DataType.R_SIGN_LANE);
        // 没有关系
    ...
    // 选中divider
    context.event.on('selected.effect-laneRSL_N', function (selectedEntities) {
        effect.enable && effect.apply.call(effect, context, effect.enable, _.pluck(selectedEntities, 'id'));
    });

    context.map().on('drawn.effect-laneRSL_N', function (param) {
