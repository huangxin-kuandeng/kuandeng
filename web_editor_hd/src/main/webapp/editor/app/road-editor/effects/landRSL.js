 ...
 * @Description: 高亮定位线与lane的关联关系--TYPE作用关系
 */

iD.effects = iD.effects || {};

iD.effects.LaneRSL = function (context) {

    var effect = {
        id: 'lane-rsl',
        button: 'laneRSL',
        title: t('effects.laneRSL.title'),
        iconText: 'RSL1',
        description: t('effects.laneRSL.description'),
        key: 'Shift+L',
        oldEntity: null,
        enable: false,
    ...
    // 选中divider
    context.event.on('selected.effect-laneRSL', function (selectedEntities) {
        effect.enable && effect.apply.call(effect, context, effect.enable, _.pluck(selectedEntities, 'id'));
    });

    context.map().on('drawn.effect-laneRSL', function (param) {
