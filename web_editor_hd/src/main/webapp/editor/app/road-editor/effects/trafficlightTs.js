 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2020-09-18 14:46:01
 * @Description: 
 */
...
/**
 * @description: 高亮交通灯与停止线
 * @param {type} 
 * @return: 
 */
iD.effects.TrafficlightTS = function(context) {
    var effect = {
        id: 'trafficlight-ts',
        button: 'trafficlightts',
        title: t('effects.trafficlightts.title'),
        iconText: 'TS',
        description: t('effects.trafficlightts.description'),
        key: 'Shift+L',
        oldEntity:null,
        enable: false,
        apply:function(context, open, selectedids){
        	...
        	if(divider.modelName == iD.data.DataType.TRAFFICLIGHT || divider.modelName == iD.data.DataType.OBJECT_PL) {
    ...
    function show(divider, enable){
        ...
        var dividerRelations = graph.parentRelations(divider, iD.data.DataType.R_TL_STOPLINE);
                // 没有关系
    ...
    // 选中divider
    context.event.on('selected.effect-trafficlightts', function(selectedEntities){
    	effect.enable && effect.apply.call(effect, context, effect.enable, _.pluck(selectedEntities, 'id'));
    });

    context.map().on('drawn.effect-trafficlightts', function(param){
