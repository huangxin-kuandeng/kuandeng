iD.ui.TagEditor.DividerAttributeTagEditor = function(context) {

    var dispatch = d3.dispatch('change');
    // var DISPLAY_FIELD_LIST = ['VIRTUAL', 'COLOR', 'TYPE', 'DRIVE_RULE', 'OVERLAY'];

    function tagEditor(selection, opts) {
		
        var tpl = iD.ui.TagEditorTpl.DividerAttribute(),
            tags = opts.tags,
            entityID = opts.id,
    ...
    tagEditor.available = function(entity) {
        return entity.modelName == iD.data.DataType.DIVIDER_ATTRIBUTE;
    }
    
    function bindFieldFilterFunc(fieldObj){
    	if(fieldObj.fieldName == 'TYPE'){
    		// 3人工虚拟分隔线时，分隔线类型显示19以后，否则不显示
    		// 1非虚拟分隔线/2路口虚拟分隔线/3缺失虚拟分隔线 时，分隔线类型显示33
    		fieldObj.optionFilter = function(optionList, d){
    			let list = [];
	        	for(let o of optionList){
	        		...
	        		*/
	        		// 限制20到31的选项，只有在VIRTUAL==3时可选
	        		// if(v != '32' && d.VIRTUAL != "3" && v > 19 && v < 33){
	        		// 	continue ;
	        		// }
	        		list.push(o);
	        	}
