...
var IMAGESPATH = '../../dist/img/dialog/'; //图片路径配置
...
_remove = function (arr,s, dust) { //如果dust为ture，则返回被删除的元素
    if (dust) {
        var dustArr = [];
        for (var i = 0; i < arr.length; i++) {
            if (s == arr[i]) {
                dustArr.push(arr.splice(i, 1)[0]);
            }
        }
        return dustArr;
    }
...
Dialog.prototype.attachBehaviors = function () {
	// 未知原因，window.Drag为true
    if (this.Drag && topWin.Drag && topWin.Drag.init) topWin.Drag.init(topWin.$id("_Draghandle_" + this.ID), topWin.$id("_DialogDiv_" + this.ID)); //注册拖拽方法
...
Dialog.prototype.create = function () {
    ...
    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
       <td height="35px" style="background-color: #F6F6F6;color:#000;font-size:14px;"><div style="padding: 9px 0 4px 14px; float: left; font-weight: bold; color:#333333;font:14px;"><span id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" style="margin-top:5px;margin-right:10px; position: relative; cursor: pointer; float: right; height: 20px; width: 20px; background-image: url(' + IMAGESPATH + 'close.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
        </tr>\
    <tr valign="top">\
        <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          ...
          <tr>\
            <td valign="top" align="center" style="position: relative;"><div id="_Container_' + this.ID + '" style="position: relative; width: ' + this.Width + 'px; height: ' + this.Height + 'px;">\
	...
	' + (function (obj) {
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="48"><div id="_DialogButtons_' + this.ID + '" style="padding-bottom:12px; text-align: right; background-color:#fff;">\
                <input type="button" class="dialog_buttonStyle" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" class="dialog_cancel_buttonStyle" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
      </tr>\
...
Dialog.prototype.close = function () {
    ...
    if (this == topWin.Dialog._Array[topWin.Dialog._Array.length - 1]) {
        ...
        _remove(topWin.Dialog._Array,this);
    }
    if (this.InvokeElementId) {
...
Dialog.alert = function (msg, func,title, w, h, opts) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog(_.extend({
        Width: w,
        Height: h
    }, opts));
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0"  style="padding:2px;vertical-align: middle;margin:2px">\
		<tr>\
		    <td align="right" style="vertical-align:middle"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.png" style="padding-right:4px;"></td>\
			<td align="left" id="Message_' + this.ID + '" class = "dialog_msg">' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.style.display = "none";
    diag.cancelButton.setAttribute("class","dialog_buttonStyle");
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right" style="vertical-align:middle;padding: 2px;"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_query.gif" style="padding-right:4px;"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:9pt;vertical-align:middle">' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.focus();
 ...
 * @param {Object} opts
 */
Dialog.sidebarAlert = function(content, opts){
	if(!content){
		return ;
	}
	opts = _.extend({
	...
	let $panel = $body.select('#_Dialog_Sidebar');
	if($panel.size() == 0){
		$panel = $body.append('div').attr("id", "_Dialog_Sidebar").attr("class", "_Dialog_Sidebar_list").style();
	}
	let $allitem = $panel.selectAll("div.dialog_listitem");
	let indexList = [];
	$allitem.each(function(){
		indexList.push(parseInt(this.getAttribute("list-index")) || 0);
	});
	...
	// console.log("正在显示的提示: " + $allitem.size() + "个, 当前提示插入: " + itemNum, indexList);
	
	let $info = $panel.append("div").html(content)
		.attr("class", "dialog_listitem bg-" + opts.type)
		.attr("title", content)
		.attr("list-index", itemNum);
	
	let $self = $($info.node());
	$info.style({
		left: -($self.outerWidth() + 5) + "px",
		top: ($self.outerHeight() + 10) * itemNum  + "px"
	});
