 ...
 * @LastEditors: tao.w
 * @LastEditTime: 2020-12-29 15:41:32
 * @Description: 
 */
iD.svg.Pic = function(proj, context) {
    ...
    // goodCacheCtx.save();
    // goodCacheCtx.lineWidth = 2;
    // goodCacheCtx.fillStyle = 'green'; 
    // // goodCacheCtx.strokeStyle = '#808080';
    // goodCacheCtx.beginPath();
    // goodCacheCtx.arc(_r, _r , _rr, 0, 2 * Math.PI);
    // // goodCacheCtx.stroke();
    // goodCacheCtx.fill();
    // goodCacheCtx.restore();
    ...
    function initDvrPlayer(context) {
        ...
        picPlayer = new iD.svg.PicPlayer(context);
        //全局变量，不要删除
        window._pic = picPlayer;
        context._pic = picPlayer;
        picPlayer.init(context.container());
        picPlayer.on('close.player', function() {
            destroyPlayer();
        });

        picPlayer.on('picUpdate', function(updaeNodeid) {
        ...
        context.event.on('track_highlight.picCanvas',(d)=>{
            // console.log(d);
            highlight_track = d.trackId;
            redrawCanvas(true)
        })
        
        function redrawCanvas(rander){
            let isEqual = true,
            ...
            if(rander) isEqual = true;

            if(picPlayer && picPlayer.pic_point && isEqual){
                //drawPic(picPlayer.allNodes,picPlayer.pic_point); //Tilden注释
                clearCanvas();
                // resetSize();
        ...
        surface.on('mousedown.pic-canvas', function (d) {
            ...
            surface.on('mouseup.pic-canvas', function (d) {
                if(lastMouseTarget == this){
                ...
                if (d3.event.target === lastMouseTarget &&
                    ...
                    clickTimeId = setTimeout(function () {
                        ...
                        if(altKeyDown){
                            ...
                            let dis = $trackcanvas.style('display');
                            if (dis == 'block') {
                                resetLoc()
                            }
                        }
                ...
                surface.on('mouseup.pic-canvas', null);

            });
        }).on('mousemove.pic-canvas', function(){
            var lastMouseTarget = d3.event.target;
            if(lastMouseTarget == this){
    ...
    function resetLoc(){
		...
		var result = getNearestNode([allNodes[dist.i], allNodes[dist.i + 1]], loc,dist.i);
    ...
    function getNearestNode(nodes, loc,i){
    ...
    function drawTimePoint(point,isPan = false) {
        ...
        // let l_width = d3.selectAll('#pic-line-canvas').node().width;
        // let l_height = d3.selectAll('#pic-line-canvas').node().height;
        // if((l_width != width || l_height != height) && picPlayer){
        //     resetSize();
        //     drawPic(picPlayer.allNodes,picPlayer.pic_point);
        // }
    ...
    function redrawDvr(trackObj, tI, isFirst) {
		...
		var trackInfo = picPlayer && picPlayer.trackPointInfo;
		var trackPointIndex = 0;
		if( trackInfo && (trackObj.trackId == trackInfo.trackId) && (iD.Task.d.task_id == trackInfo.taskId) ){
			trackPointIndex = _.pluck(trackObj.nodes, 'id').indexOf(trackInfo.trackPointId);
		}

        initDvrPlayer(context);
        picPlayer.init(context.container());  
        picPlayer.renderPicPlayer(trackObj, trackObj.nodes[trackPointIndex].id).show();
        // resetSize();
    ...
    function drawPic(points,anchorPoint){
        ...
        //trackLine(lineCanvas,points.allPoints);
    ...
    function clearCanvas(){
        ...
        // lineCanvas.clearRect(0, 0, width, height);
        // trackCanvas.clearRect(0, 0, width, height);
        // anchorCanvas.clearRect(0, 0, width, height);
    ...
    function trackPoint(cxt,points) {
        ...
        function ctxDrawNode(cvs, pixel, _ctx, node){
            ...
            // }else 
            if(_ctx && node && node.tags && highlight_track ==  node.tags.trackId){
                var grey = '#ffeb3b';
                // if(_ctx.fillStyle != grey){
                    ctxPointFill(_ctx, grey);
                // }
        ...
        //     ctxDrawNode(goodCacheCanvas, node, acceptableCacheCtx, pointClassification.goodPoints[i]); 
        // }
    ...
    dvrRender.setTracks = function(_){
        ...
        _tracks = _;
        // if (_tracks.length > 255) {
        //     _tracks = _tracks.splice(0, 255);
        // }
