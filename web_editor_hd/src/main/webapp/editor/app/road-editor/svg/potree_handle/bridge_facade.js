...
;(function(exp){
    let handle = exp[iD.data.DataType.ROAD_FACILITIES] = {
        context: null,
        potree: null,
        viewer: null
    };
    ...
    handle.onDrag = function(data){
        ...
        // 更新地图位置
        let actions = [];
        crossLoc = toMap.forward([crossPos.x, crossPos.y, crossPos.z]);
        ...
        let updateLocs = _.sortBy(newLocs, function(item) {
            return -item.loc[2];
        });

        let copyLocs2 = updateLocs.find(d => {
            return (updateLocs[2].index + d.index == 3);
        });
        let copyLocs3 = updateLocs.find(d => {
            return (updateLocs[3].index + d.index == 3);
        });
        ...
        updateLocs.forEach(d => {
            actions.push(iD.actions.MoveNode(entity.nodes[d.index], d.loc));
        });
        actions.push('更改桥立面');
    ...
    handle.getGroundAreaEntity = function(data){
        ...
        if (entity.modelName != iD.data.DataType.ROAD_FACILITIES) {
            return false;
        }
		
		if (entity.tags.DIRECTION != "1"){
            return false;
		}
        // 是否是地面区域
