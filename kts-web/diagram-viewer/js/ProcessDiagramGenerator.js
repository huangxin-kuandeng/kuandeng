...
var ProcessDiagramGenerator = {	
	...
	init: function(){
		...
		this.activityDrawInstructions["callActivity"] = function(){
			...
			processDiagramCanvas.drawcallActivity(activityImpl.getProperty("name"), activityImpl.getX(), activityImpl.getY(), activityImpl.getWidth(), activityImpl.getHeight());
		};
...
//		};
	...
	generateDiagram: function(processDefinitionDiagramLayout){
		...
		var processDefinitionKey = processDefinitionDiagramLayout.processDefinition.key;
//		console.log("Init canvas ", processDefinitionId);
		if( processDefinitionKey=="DataHandleEditCheckProcess"){
			this._report = 1;
		}else{
			this._report = 0;
		}
		...
		for(var i=0; i<processDefinitionDiagramLayout.activities.length; i++){
			var str = /[\u4e00-\u9fa5]/;
			if( (str.test(processDefinitionDiagramLayout.activities[i].properties.name)) || (processDefinitionDiagramLayout.activities[i].properties.name == "Error") ){
				this._activities.push(processDefinitionDiagramLayout.activities[i]);
			}else{
...
/*
 * 之前的执行顺序为  环节高亮-->箭头高亮;
 * 会出现异常处理/重新触发的环节时-当前环节会被箭头覆盖(即动态图会被执行过的蓝色图片覆盖);
 * 执行顺序修改之后,反过来覆盖。
 */
...
//		箭头高亮显示  已完成的   绿色
		...
		for (var i in highLights.flows) {
			...
			if(!object){
				continue;				//箭头高亮找不到该对象----跳出本次循环开始下一个
			}
...
//		环节高亮显示  正在执行的环节为动态图片
	...
	addBreadCrumbsItem: function(processDefinitionId){
		...
		if( liListNew.size ){
			var liListNewSize = liListNew.size();
		}else{
			var liListNewSize = liListNew.length;
		}
		
		//console.log("liListNew size: ", liListNew.size());
		var values = {
			id: 'breadCrumbsItem_' + liListNewSize,
