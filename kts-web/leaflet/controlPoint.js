...
//		打开控制点分组列表
		$(".rightBtn").click(function(e) {
			$(".leftTasks").toggle();
			controlPoint.countPoint();
			controlPoint.groupNameList();
		});
//		关闭控制点分组列表
		$(".leftTasks span.glyphicon").click(function(e) {
			$(".leftTasks").toggle();
		});
//		关闭图片展示
...
//		图片--正常图片
		$(".pointImage .preImage").click(function(e) {
			var _type = this.getAttribute('data-type');
			controlPoint.changeImage(_type);
		});
//		图片--缩略图
		$(".pointImage .nextImage").click(function(e) {
			var _type = this.getAttribute('data-type');
			controlPoint.changeImage(_type);
		});
//		点击分组查看控制点
...
//	控制点图片的切换
	changeImage: function(type){
		var _url = $(".pointImage img")[0].src,
			newUrl = _url.split('&')[0],
			changeUrl = newUrl + '&type='+type;
		$(".pointImage img")[0].src = changeUrl;
	},
	
//	查询控制点的数量
	countPoint: function(){
		$("#loading").css("display","block");
		var _url = configURL.krs+"controlPoint/count?kieVersion=empty";
		$.getAjax(_url, {}, function(data){
			...
			if(data.code != '0'){
				...
				let _count = data.result || 0;
				$('.leftTasks .pointCount').html(_count);
			}
		})
	},
...
//	查询控制点分组列表--查询
	groupNameList: function(){
		$("#loading").css("display","block");
		var _url = configURL.krs+"controlPoint/groupNames/query/v2?kieVersion=empty";
		$.getAjax(_url, {}, function(data){
			$("#loading").css("display","none");
			var _list = [];
	        ...
	        var listModel = `
	            ${_list.map(m => `
		            <tr class="tooltipName">
		            	<td title="${m.groupName}">${m.groupName}</td>
		            	<td title="${m.count}">${m.count}</td>
		            	<td class="groupNameOpen">
		            		<a class="queryPoint" data-name="${m.groupName}" style="cursor:pointer;">控制点</a>
		            		<a class="deletePoint" data-name="${m.groupName}" style="cursor:pointer;">删除</a>
		            	</td>
		            </tr>
	            `).join('')}
	        ` || "";
...
//	删除控制点
	deletePoint: function(id,leafletId=false){
		var htmlText = leafletId ? '当前' : (id+'分组'),
			html_1 = [
				"是否删除"+htmlText+"控制点"
			];
		$.openTips_btn(html_1, function(){
			let param = leafletId ? ('controlPoint/delete?kieVersion=empty&ids='+id) : ('controlPoint/delete?kieVersion=empty&groupName='+encodeURIComponent(id));
			$(".modal1").hide();
			$("#loading").css("display","block");
			var _url = configURL.krs+param;
			$.getAjax(_url,true,function(data){
				...
				if(data.code == '0'){
					_text = '删除成功';
					_type = true;
					var _layer = leafletId ? (leafletMap.editTools.featuresLayer._layers[leafletId] || leafletMap._layers[leafletId]) : null;
					if(_layer){
						_layer.remove();
					}else{
		...
		$.getAjax(_url,true,function(data,group){
	        ...
	        for(var i=0; i<data.result.length; i++){
	        	var imgId = data.result[i].properties.imgId,
	        		...
	        		className = imgId ? 'ImageIcon' : 'noImageIcon',
	        		featureColor = (featureType != '99' && imgId) ? 'specialImageIcon' : className;
	        		
	        	noImageIcon.options.className = featureColor;
	        	
	        	var properties = [];
	        	var imgaeBtn = imgId ? `<p class="openImage pStyle"><a href="#" onclick="controlPoint.openImage('${imgId}')">查看图片</a></p>` : '';
				...
				node = [data.result[i].geometry.coordinates[1], data.result[i].geometry.coordinates[0]];
			 	...
			 	var point = L.marker(node, {
				    ...
				    html_type: _typeContent
			  		...
			  		var deleteHtml = `
			  			<p class="deletePoint pStyle"><a href="#" onclick="controlPoint.deletePoint('${layer.options.pointId}','${layer._leaflet_id}')" title="删除当前控制点">删除</a></p>
			  		`;
				    controlPoint.catchFeatureType = layer;
				    var _html = layer.options.html_type + layer.options._html + deleteHtml;
					return _html;
				}).addTo(leafletMap);
